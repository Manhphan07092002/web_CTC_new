      ...
      <div className="container mx-auto px-6 py-16">
        <div className="grid lg:grid-cols-2 gap-16 items-center mb-20">
          <div>
            <div className="inline-flex items-center gap-2 bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-4">
              <Sun size={14} /> {isEn ? 'Solar EPC – Turnkey Contractor' : 'Solar EPC – Tổng thầu'}
            </div>
            <h2 className="text-3xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-5 leading-tight">
              {isEn ? 'Rooftop Solar Systems' : 'Điện Mặt Trời Áp Mái'}<br />
              <span className="text-orange-500">{isEn ? 'Residential & C&I' : 'Hộ Gia Đình & C&I'}</span>
            </h2>
            <p className="text-slate-600 dark:text-slate-300 text-base leading-relaxed mb-6 text-justify">
              {isEn ? (
                'CTC provides turnkey EPC services for rooftop solar power systems for households and Commercial & Industrial (C&I) clients. With extensive experience across Vietnam, CTC guarantees quality, schedule, and optimal ROI.'
              ) : (
                'CTC cung cấp dịch vụ EPC trọn gói hệ thống điện mặt trời áp mái cho hộ gia đình và khách hàng Thương mại & Công nghiệp (C&I). Với kinh nghiệm thi công hàng trăm công trình tại miền Trung, CTC đảm bảo chất lượng, tiến độ và hiệu quả đầu tư tối ưu.'
              )}
            </p>
            <div className="grid grid-cols-2 gap-4">
              {[
                { val: '500+', label: isEn ? 'Solar Projects Constructed' : 'Công trình Solar đã thi công' },
                { val: '4–5 yrs', label: isEn ? 'Typical ROI Period' : 'Thời gian hoàn vốn điển hình' },
                { val: '20+', label: isEn ? 'Years System Lifespan' : 'Năm vòng đời hệ thống' },
                { val: '100%', label: isEn ? 'On-time Commitment' : 'Cam kết đúng tiến độ' },
              ].map((s, i) => (
            ...
            <div className="absolute -bottom-5 -left-5 bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-4 border border-orange-100 dark:border-orange-900/50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-slate-500 dark:text-slate-400">{isEn ? 'Ministry of Construction' : 'Bộ Xây dựng – 2020'}</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-20">
          <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-8 text-center">
            {isEn ? 'Featured Customer Models' : 'Mô Hình Khách Hàng Tiêu Biểu'}
          </h3>
          <div className="grid md:grid-cols-3 gap-6">
            {[
              {
                icon: <Building2 size={24} />,
                title: isEn ? 'Residential & Villas' : 'Hộ gia đình & Biệt thự',
                desc: isEn ? '5–30kWp systems connected to the grid. 4–5 year payback period, saving 100% electricity bills afterwards.' : 'Hệ thống 5–30kWp, hòa lưới bán điện EVN. Hoàn vốn 4–5 năm, tiết kiệm toàn bộ hóa đơn điện sau đó.',
                color: 'blue',
              },
              {
                icon: <Factory size={24} />,
                title: isEn ? 'Factories & Industrial Parks' : 'Nhà máy & Khu công nghiệp',
                desc: isEn ? '100kWp – 5MWp C&I scale systems. Reduces manufacturing power costs and achieves green ESG/LEED certificates.' : 'Hệ thống C&I quy mô 100kWp – 5MWp. Giảm chi phí điện sản xuất, đạt chứng chỉ xanh ESG/LEED.',
                color: 'orange',
              },
              {
                icon: <Zap size={24} />,
                title: isEn ? 'Office Buildings & Commercial' : 'Tòa nhà văn phòng & Thương mại',
                desc: isEn ? 'Integrated BESS energy storage, optimizing peak hours and stabilizing operational power supply.' : 'Tích hợp hệ thống BESS lưu trữ điện, tối ưu giờ cao điểm và ổn định nguồn điện vận hành.',
                color: 'yellow',
              },
            ].map((m, i) => (
        ...
        <div className="grid lg:grid-cols-2 gap-12 mb-16">
          <div className="bg-gradient-to-br from-corporate to-blue-800 text-white rounded-3xl p-8">
            <h3 className="text-xl font-extrabold mb-6 flex items-center gap-3">
              <CheckCircle className="text-green-400" size={24} /> {isEn ? 'Key Benefits' : 'Lợi Ích Vượt Trội'}
            </h3>
            <ul className="space-y-4">
              {(isEn ? [
                'Save 70–100% of monthly electricity bills',
                'Generate revenue from selling excess power to EVN',
                'Reduce roof temperature by 3–5°C, saving AC power',
                'Environmental protection – Reduce CO₂ emissions',
                '25–30 year panel lifespan with long-term warranty',
                'Increase real estate value by up to 4%',
              ] : [
          ...
          <div>
            <h3 className="text-xl font-extrabold text-slate-900 dark:text-white mb-6 flex items-center gap-3">
              <TrendingUp className="text-orange-500" size={24} /> {isEn ? 'CTC EPC Process' : 'Quy Trình EPC CTC'}
            </h3>
            <div className="space-y-4">
              {(isEn ? [
                { step: '01', title: 'Consultation & Survey', desc: 'Assess roof condition, solar radiation, power load, and propose optimal capacity.' },
                { step: '02', title: 'Technical Design', desc: 'Detailed engineering drawings, equipment selection, and EVN submission.' },
                { step: '03', title: 'Construction & Installation', desc: 'Professional technical team, on-schedule execution with absolute safety.' },
                { step: '04', title: 'Acceptance & Commissioning', desc: 'Grid connection, full system testing, and complete documentation handover.' },
              ] : [
        ...
        <div className="text-center bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800/50 rounded-3xl p-10">
          <h3 className="text-2xl font-extrabold text-slate-900 dark:text-white mb-3">
            {isEn ? 'Get Your Free Quote Today' : 'Nhận Báo Giá Miễn Phí Ngay Hôm Nay'}
          </h3>
          <p className="text-slate-500 dark:text-slate-400 mb-6">
            {isEn ? 'CTC engineering team is ready to consult and design the best system tailored to your needs.' : 'Đội ngũ kỹ sư CTC sẵn sàng tư vấn và thiết kế hệ thống phù hợp nhất với nhu cầu của bạn.'}
          </p>
