/**
 * Seed/Crawler V6.1: mục tiêu 850 sản phẩm thật đang có trên thị trường,
...
const DISCOVERY_PARSER_VERSION = '6.1.0';
const SEED_TAG = 'ctc-seed-850-real-products-geo-evidence-v6-1';
...
const DISCOVERY_SCAN_CACHE_FILE = path.join(CACHE_DIR, 'discovery-scan-cache-v6-1.json');
...
async function discoverCategory(targetItem: CategoryTarget, excludedIdentities: ReadonlySet<string>): Promise<DiscoveredProduct[]> {
  ...
  const fromSitemaps = await sitemapPromise;
...
async function discoverProductCatalog(): Promise<CatalogGroup[]> {
  ...
  for (const targetItem of CATEGORY_TARGETS) {
    ...
    console.log(
      ...
      `| search=${diagnostics.searchResults}, sitemap=${diagnostics.sitemapUrls}, fetch=${diagnostics.pagesFetched}` +
...
async function main(): Promise<void> {
  ...
  if (VALIDATE_CONFIG_ONLY) {
    ...
    console.log(`✅ Cấu hình V6.1 hợp lệ: ${CATEGORY_TARGETS.length} danh mục lá, quota ${TARGET_TOTAL_PRODUCTS}.`);
    console.log(JSON.stringify(totals, null, 2));
    console.log(`🔎 Trần request Serper: ${MAX_DISCOVERY_QUERIES}; ước tính tối đa theo cấu hình: ${estimatedDiscoveryRequests()}.`);
    return;
  }
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('CTC — V6.1: 850 SẢN PHẨM THẬT + CATEGORY SEO + EVIDENCE + ẢNH ĐÚNG MODEL');
  console.log('════════════════════════════════════════════════════════════');
  ...
  console.log(`\n✅ Hoàn thành V6.1: upsert ${prepared.length} sản phẩm thật, Category SEO, evidence/datasheet/specs và ảnh đúng model.`);
}
