 ...
 *
 * Chạy local:
 *   npx tsx server/scripts/seed-500-seo-products-real-img-geo-v7.ts
 *
 * Chạy Docker:
 *   docker compose exec app npx tsx server/scripts/seed-500-seo-products-real-img-geo-v7.ts
 */
...
const SEED_TAG = 'ctc-seed-500-seo-geo-real-image-v7';
...
const CACHE_DIR = path.resolve(__dirname, '../.cache/seed-500-products-v7');
const IMAGE_CACHE_FILE = path.join(CACHE_DIR, 'image-cache-v7.json');
const SOURCE_CACHE_FILE = path.join(CACHE_DIR, 'source-cache-v7.json');
...
function assertNoDuplicateImages(images: Map<string, VerifiedImage>): void {
  ...
  if (duplicates.length > 0) {
    const sample = duplicates.slice(0, 5)
      .map(([identity, products]) => `- ${identity}: ${products.join(' | ')}`)
      .join('\n');
    console.warn(`⚠️ Lưu ý: Có ${duplicates.length} sản phẩm chia sẻ nhóm ảnh danh mục:\n${sample}`);
  }
}
