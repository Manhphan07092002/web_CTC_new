/**
 * Seed 400 sản phẩm thực tế với nội dung chuẩn SEO
 *
 * Dữ liệu được tạo từ file "danh_muc_400_san_pham_thuc_te.xlsx".
 * Không sinh tên giả kiểu CTC-Pro Series.
 *
 * Cách chạy gợi ý:
 *   npx ts-node src/scripts/seed/seed-400-real-products.ts
 *
 * Xóa toàn bộ sản phẩm trước khi seed:
 *   CLEAR_ALL_PRODUCTS=true npx ts-node src/scripts/seed/seed-400-real-products.ts
 *
...
const IMAGE_BY_CATEGORY: Record<string, string> = {
  router:
    'https://images.unsplash.com/photo-1544197150-b99a580bb7a8?w=1200&q=85',
  switch:
    'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=1200&q=85',
  'wi-fi-access-point':
    'https://images.unsplash.com/photo-1606904825846-647eb07f5be2?w=1200&q=85',
  'thiet-bi-can-bang-tai':
    'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=1200&q=85',
  'module-quang-sfp-qsfp':
    'https://images.unsplash.com/photo-1544197150-b99a580bb7a8?w=1200&q=85',
  odf:
    'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=1200&q=85',
  'voip-gateway':
    'https://images.unsplash.com/photo-1580894732468-058f747280f2?w=1200&q=85',
  'ip-pbx':
    'https://images.unsplash.com/photo-1580894732468-058f747280f2?w=1200&q=85',
  'dien-thoai-ip':
    'https://images.unsplash.com/photo-1580894732468-058f747280f2?w=1200&q=85',
  ...
  'may-chu':
    'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=1200&q=85',
  pc:
    'https://images.unsplash.com/photo-1587831990711-23ca6441447b?w=1200&q=85',
  'mini-pc':
  ...
  'tam-pin-nang-luong-mat-troi':
    'https://images.unsplash.com/photo-1509391365360-2e959784a276?w=1200&q=85',
  'bo-hoa-luoi-inverter':
    'https://images.unsplash.com/photo-1509391365360-2e959784a276?w=1200&q=85',
  'ac-quy-chi':
    'https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?w=1200&q=85',
  'ac-quy-lithium':
    'https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?w=1200&q=85',
  'ac-quy-nuoc':
    'https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?w=1200&q=85',
...
function getImageForCategory(categoryName: string): string {
  const normalizedCategory = normalizeText(categoryName);
  return (
    IMAGE_BY_CATEGORY[normalizedCategory] ||
    'https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=1200&q=85'
  );
}
...
async function seed400RealProducts(): Promise<void> {
  try {
    ...
    if (categories.length === 0) {
      throw new Error(
        'Không tìm thấy danh mục. Hãy chạy seed danh mục trước.',
      );
    }

    console.log(`📁 Found ${categories.length} active categories.`);

    const unmatchedCategoryNames = Array.from(
      new Set(
        catalogProducts
          .filter(
            (product) =>
    ...
    if (unmatchedCategoryNames.length > 0) {
      console.error('\n❌ Không khớp được các danh mục sau:');
      ...
      throw new Error(
        'Seed đã dừng trước khi ghi dữ liệu để tránh gán sai danh mục.',
      );
    }
    ...
    } else if (RESET_CATALOG_PRODUCTS) {
    ...
    const productsToInsert = catalogProducts.map(
      (catalogProduct, index) => {
        ...
        const imageUrl = getImageForCategory(
