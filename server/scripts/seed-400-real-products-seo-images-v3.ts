/**
 * Seed 400 sản phẩm thực tế, nội dung SEO an toàn và ảnh đúng model (V3)
 *
 * Dữ liệu được tạo từ file "danh_muc_400_san_pham_thuc_te.xlsx".
 * Không sinh tên giả kiểu CTC-Pro Series và không tự suy diễn thông số kỹ thuật.
 *
 * Cách chạy đầy đủ:
 *   npx tsx server/scripts/seed-400-real-products-seo-images-v3.ts
 *
 * Kiểm tra dữ liệu, không kết nối MongoDB:
 *   VALIDATE_ONLY=true npx tsx server/scripts/seed-400-real-products-seo-images-v3.ts
 *
 * Chỉ cập nhật nội dung/brand/model cho các sản phẩm CTC-REAL-* hiện có:
 *   UPDATE_CONTENT_ONLY=true npx tsx server/scripts/seed-400-real-products-seo-images-v3.ts
 *
 * Xóa toàn bộ sản phẩm trước khi seed:
 *   CLEAR_ALL_PRODUCTS=true npx tsx server/scripts/seed-400-real-products-seo-images-v3.ts
 *
 ...
 *   MIN_PRODUCT_IMAGES=1                   Số ảnh tối thiểu trước khi dùng nguồn bổ sung
 ...
 *   ENABLE_BING_IMAGE_SEARCH=false         Mặc định tắt để tránh lấy nhầm ảnh
 *   OFFICIAL_IMAGES_ONLY=true              Chỉ nhận ảnh từ trang/nguồn chính thức
 *   ALLOW_SHARED_IMAGES=false              Không dùng lại cùng một ảnh cho nhiều sản phẩm
 *   UPDATE_IMAGES_ONLY=true                Chỉ cập nhật image/images
 */
...
const VALIDATE_ONLY =
  String(process.env.VALIDATE_ONLY || '').toLowerCase() === 'true';

const UPDATE_CONTENT_ONLY =
  String(process.env.UPDATE_CONTENT_ONLY || '').toLowerCase() === 'true';
...
const CANONICAL_BRAND_MAP: Record<string, string> = {
  'tp-link-omada': 'TP-Link',
  'dell-poweredge': 'Dell',
  'dell-optiplex': 'Dell',
  'lenovo-thinkcentre': 'Lenovo',
  'lenovo-thinkpad': 'Lenovo',
  'huawei-fusionsolar': 'Huawei',
  'byd-battery-box': 'BYD',
};

const PRODUCT_LINE_MAP: Record<string, string> = {
  'tp-link-omada': 'Omada',
  'dell-poweredge': 'PowerEdge',
  'dell-optiplex': 'OptiPlex',
  'lenovo-thinkcentre': 'ThinkCentre',
  'lenovo-thinkpad': 'ThinkPad',
  'huawei-fusionsolar': 'FusionSolar',
  'byd-battery-box': 'Battery-Box',
};
...
function getCanonicalBrand(brand: string): string {
  ...
  return CANONICAL_BRAND_MAP[normalizedBrand] || brand.trim();
}

function getProductLine(brand: string): string | undefined {
  return PRODUCT_LINE_MAP[normalizeText(brand)];
}
...
function validateCatalogProducts(
  ...
  if (products.length !== 400) {
    errors.push(`Danh mục phải có đúng 400 sản phẩm, hiện có ${products.length}.`);
  }
  ...
  for (const field of uniqueFields) {
    ...
    products.forEach((product, index) => {
      const normalizedValue = normalizeCompact(product[field]);
      const previousIndex = seen.get(normalizedValue);

      if (previousIndex !== undefined) {
        errors.push(
          `Trùng ${field}: sản phẩm #${previousIndex + 1} và #${index + 1} (${product[field]}).`,
        );
      } else {
  ...
  products.forEach((product, index) => {
    const position = `#${index + 1} ${product.name || '(không có tên)'}`;

    for (const field of requiredFields) {
      if (!String(product[field] ?? '').trim()) {
        errors.push(`${position}: thiếu trường ${field}.`);
      }
    }

    if (normalizeText(product.brand) === 'ctc') {
      errors.push(`${position}: brand không được gán mặc định là CTC.`);
    }

    if (/ctc-pro|ctc\s+series/i.test(product.name)) {
      errors.push(`${position}: phát hiện tên sản phẩm giả/tự sinh.`);
    }

    if (
      /(?:63\s+tỉnh|53\s+kỹ\s+sư|1\s+đổi\s+1|ce,\s*rohs|iso\s*9001)/i.test(
        `${product.shortDescription} ${product.verificationNote}`,
      )
    ) {
      errors.push(`${position}: chứa tuyên bố mẫu chưa được kiểm chứng.`);
    }

    try {
      const source = new URL(product.sourceUrl);
      if (source.protocol !== 'https:') {
        warnings.push(`${position}: sourceUrl không dùng HTTPS.`);
      }
    } catch {
      errors.push(`${position}: sourceUrl không hợp lệ (${product.sourceUrl}).`);
    }

    if (
      !normalizeCompact(product.name).includes(
        normalizeCompact(product.model),
      )
    ) {
      warnings.push(`${position}: tên sản phẩm chưa chứa đầy đủ model.`);
    }

    const canonicalBrand = getCanonicalBrand(product.brand);
    if (!canonicalBrand) {
      errors.push(`${position}: không xác định được thương hiệu chuẩn.`);
    }
  });
...
function scoreImageCandidate(
  ...
  const imageUsesOfficialHost = isSameHostOrSubdomain(
    imageUrl,
    product.sourceUrl,
  );
  const pageUsesOfficialHost = isSameHostOrSubdomain(
    pageUrl,
    product.sourceUrl,
  );

  if (imageUsesOfficialHost) {
    score += 35;
  } else if (pageUsesOfficialHost && origin !== 'bing-image') {
    // Nhiều hãng phục vụ ảnh qua CDN khác domain. Chỉ tin CDN khi ảnh
    // được nhúng trực tiếp trên một trang chính thức của hãng.
    score += 20;
  } else if (pageUsesOfficialHost && origin === 'bing-image') {
    score += 5;
...
function buildSeoShortDescription(product: CatalogProduct): string {
  ...
  const canonicalBrand = getCanonicalBrand(product.brand);
  const productLine = getProductLine(product.brand);
  const lineText = productLine ? `, dòng ${productLine}` : '';

  const variants = [
    `${product.name} (${product.model}) là ${profile.singularLabel} thương hiệu ${canonicalBrand}${lineText}, phù hợp cho ${profile.primaryUse}. Liên hệ CTC Telecom để kiểm tra cấu hình và báo giá.`,
    `Tìm hiểu ${product.name} model ${product.model} của ${canonicalBrand}${lineText}: sản phẩm thuộc nhóm ${product.categoryName.toLowerCase()} dành cho ${profile.primaryUse}.`,
    `${product.name} thuộc nhóm ${product.categoryName}, hỗ trợ ${profile.solution}. CTC Telecom hỗ trợ đối chiếu đúng model, khả năng tương thích và tình trạng phân phối.`,
    `Báo giá ${product.name} – model ${product.model}, thương hiệu ${canonicalBrand}${lineText}. Thông số và chính sách bảo hành cần được xác nhận theo đúng phiên bản tại thời điểm báo giá.`,
  ];
...
function getFeatures(product: CatalogProduct): string[] {
  ...
  return [
    ...
    `Thương hiệu: ${canonicalBrand}`,
    ...(productLine ? [`Dòng sản phẩm: ${productLine}`] : []),
    ...profile.benefits,
    'Cần đối chiếu datasheet đúng model, phụ kiện, khả năng tương thích và tình trạng phân phối trước khi đặt hàng',
  ];
}
...
function generateSeoDescription(product: CatalogProduct): {
  ...
  const canonicalBrand = getCanonicalBrand(product.brand);
  const productLineValue = getProductLine(product.brand);
  const brand = escapeHtml(canonicalBrand);
  const productLine = productLineValue
    ? escapeHtml(productLineValue)
    : undefined;
...
${applicationItems}
    ...
    <table class="w-full border-collapse border border-gray-200 dark:border-slate-700 text-sm">
      <tbody>
        ...
        ${
          productLine
            ? `<tr class="border-b border-gray-200 dark:border-slate-700">
          <td class="p-2.5 font-bold bg-gray-50 dark:bg-slate-800">Dòng sản phẩm</td>
          <td class="p-2.5">${productLine}</td>
        </tr>`
            : ''
        }
        <tr class="border-b border-gray-200 dark:border-slate-700">
  ...
  <p class="leading-relaxed">
    <strong>Lưu ý:</strong> ${verificationNote} Nội dung trên là thông tin nhận diện và hướng dẫn lựa chọn; không thay thế datasheet của đúng model. Thông số, phụ kiện, hình ảnh và chính sách bảo hành phải được xác nhận lại tại thời điểm báo giá.
  </p>
  ...
  return {
    ...
    specifications: [
      ...
      `Model: ${product.model}`,
      `Thương hiệu: ${canonicalBrand}`,
      ...(productLineValue ? [`Dòng sản phẩm: ${productLineValue}`] : []),
      `Nhóm chính: ${product.mainGroup}`,
      `Danh mục: ${product.categoryName}`,
      `Ứng dụng tham khảo: ${profile.applications.join(', ')}`,
      `Nguồn hãng: ${product.sourceUrl}`,
      `Trạng thái thông số: Cần xác nhận theo datasheet đúng model`,
    ].join('; '),
  };
}
...
async function seed400RealProducts(): Promise<void> {
  try {
    const validation = validateCatalogProducts(catalogProducts);
    ...
    if (validation.errors.length > 0) {
      throw new Error(
        `Dữ liệu catalog không hợp lệ:\n- ${validation.errors.join('\n- ')}`,
      );
    }

    console.log(
      `✅ Catalog validation passed: ${catalogProducts.length} sản phẩm, không trùng tên/model/từ khóa.`,
    );

    if (VALIDATE_ONLY) {
      console.log('🧪 VALIDATE_ONLY=true: dừng trước khi kết nối MongoDB.');
      return;
    }

    if (UPDATE_IMAGES_ONLY && UPDATE_CONTENT_ONLY) {
      throw new Error(
        'Không được bật đồng thời UPDATE_IMAGES_ONLY và UPDATE_CONTENT_ONLY.',
      );
    }
    ...
    if (UPDATE_CONTENT_ONLY) {
      console.log(
        '\n📝 UPDATE_CONTENT_ONLY=true: cập nhật brand, model và nội dung; giữ nguyên image/images.',
      );
      ...
      for (let index = 0; index < catalogProducts.length; index += 1) {
        ...
        const canonicalBrand = getCanonicalBrand(
        ...
        const result = await Product.updateOne(
          { code },
          {
            $set: {
              ...
              manufacturer: canonicalBrand,
              model: catalogProduct.model,
              sku: catalogProduct.model,
              ...(productLine
                ? { productLine }
                : {}),
              description,
              ...
              specifications,
              warranty:
                'Liên hệ xác nhận theo đúng model và chính sách của hãng/nhà phân phối',
              features,
              focusKeyword:
                catalogProduct.focusKeyword ||
                `${catalogProduct.categoryName} ${canonicalBrand} ${catalogProduct.model}`,
              sourceUrl: catalogProduct.sourceUrl,
              verificationNote:
                catalogProduct.verificationNote,
              contactPrice: true,
              stockStatus: 'contact',
              stock: 0,
            },
      ...
      console.log(
        `✅ Content update completed. Matched: ${matchedCount}; modified: ${modifiedCount}.`,
      );
      return;
    }
    ...
    const productsToInsert = catalogProducts.map(
      (catalogProduct, index) => {
        ...
        const effectiveProduct: CatalogProduct = {
          ...catalogProduct,
          sourceUrl:
            productImageResults[index].detailPage ||
            catalogProduct.sourceUrl,
        };
        const canonicalBrand = getCanonicalBrand(
          catalogProduct.brand,
        );
        ...
        const {
          ...
          features,
          specifications,
        } = generateSeoDescription(effectiveProduct);
        ...
        return {
          ...
          shortDescription,
          specifications,
          sourceUrl: effectiveProduct.sourceUrl,
          verificationNote:
            catalogProduct.verificationNote,
          price: '0',
          originalPrice: '0',
