/**
 * Seed V8 Static Catalog: 850 sản phẩm khai báo sẵn, không crawl catalog,
 ...
 * - Danh sách 850 sản phẩm và model/part number nằm trực tiếp trong file.
 * - Không crawl sitemap, category, API catalog hoặc website hãng để tìm sản phẩm.
 * - Chỉ dùng Google Images qua Serper để tìm URL ảnh đúng model.
 * - Lưu URL ảnh từ xa vào MongoDB; mặc định không tải/mirror ảnh về server.
 ...
 *
 * Kiểm tra catalog/model, không tìm ảnh và không ghi DB:
 *   VALIDATE_ONLY=true npx tsx server/scripts/seed-850-static-products-google-image-url-v8.ts
 *
 * Tìm URL ảnh Google và dry-run:
 *   DRY_RUN=true ENABLE_SERPER=true SERPER_API_KEY=... \
 *     npx tsx server/scripts/seed-850-static-products-google-image-url-v8.ts
 */
...
const DISCOVERY_ONLY = envBool('DISCOVERY_ONLY', false);
const STATIC_SEED_MODE = true;
const FREE_DISCOVERY_MODE = false;
const ENABLE_SERPER = envBool('ENABLE_SERPER', true);
const SERPER_ENABLED = ENABLE_SERPER && Boolean(SERPER_API_KEY);
const PLAYWRIGHT_FALLBACK = envBool('PLAYWRIGHT_FALLBACK', false);
const RESET_PRODUCTS = envBool('RESET_PRODUCTS', false);
const RESET_ALL_PRODUCTS = envBool('RESET_ALL_PRODUCTS', false);
const CLEANUP_LEGACY_GLOBAL_BRAND_MENU = envBool('CLEANUP_LEGACY_GLOBAL_BRAND_MENU', true);
...
const DISCOVERY_PAGES_PER_QUERY = envInt('DISCOVERY_PAGES_PER_QUERY', 2, 1, 5);
...
const MAX_DISCOVERY_QUERIES = envInt('MAX_DISCOVERY_QUERIES', 1800, 54, 5000);
const DISCOVERY_CACHE_MAX_AGE_DAYS = envInt('DISCOVERY_CACHE_MAX_AGE_DAYS', 30, 1, 365);
const DISCOVERY_USE_SITEMAPS = envBool('DISCOVERY_USE_SITEMAPS', true);
const DISCOVERY_USE_CATEGORY_PAGES = envBool('DISCOVERY_USE_CATEGORY_PAGES', FREE_DISCOVERY_MODE);
const DISCOVERY_USE_PUBLIC_CATALOG_API = envBool('DISCOVERY_USE_PUBLIC_CATALOG_API', FREE_DISCOVERY_MODE);
const MAX_SITEMAP_FILES_PER_DOMAIN = envInt('MAX_SITEMAP_FILES_PER_DOMAIN', 12, 1, 50);
const MAX_SITEMAP_URLS_PER_DOMAIN = envInt('MAX_SITEMAP_URLS_PER_DOMAIN', 500, 20, 5000);
const MAX_CATEGORY_PAGES_PER_DOMAIN = envInt('MAX_CATEGORY_PAGES_PER_DOMAIN', 30, 5, 200);
const MAX_CATEGORY_URLS_PER_DOMAIN = envInt('MAX_CATEGORY_URLS_PER_DOMAIN', 800, 20, 5000);
const PUBLIC_API_MAX_PAGES = envInt('PUBLIC_API_MAX_PAGES', 2, 1, 10);
const DISCOVERY_GL = (process.env.DISCOVERY_GL || 'us').replace(/[^a-z]/gi, '').slice(0, 2).toLowerCase() || 'us';
const DISCOVERY_HL = (process.env.DISCOVERY_HL || 'en').replace(/[^a-z]/gi, '').slice(0, 2).toLowerCase() || 'en';
const CRAWLER_USER_AGENT = process.env.CRAWLER_USER_AGENT
...
const DISCOVERY_PARSER_VERSION = '8.0.0-static';
const SEED_TAG = 'ctc-seed-850-static-products-google-image-url-v8';
const GEO_STANDARD = 'CTC-GEO-2026.3.1';

const CACHE_DIR = path.resolve(__dirname, '../.cache/seed-850-products-v8-static');
const DISCOVERY_CACHE_FILE = path.join(CACHE_DIR, 'unused-discovery-cache-v8.json');
const DISCOVERY_SCAN_CACHE_FILE = path.join(CACHE_DIR, 'unused-discovery-scan-v8.json');
const DISCOVERY_REPORT_FILE = path.join(CACHE_DIR, 'static-catalog-report-v8.json');
const DISCOVERED_CATALOG_FILE = path.join(CACHE_DIR, 'static-catalog-v8.json');
const IMAGE_CACHE_FILE = path.join(CACHE_DIR, 'google-image-url-cache-v8.json');
const EVIDENCE_CACHE_FILE = path.join(CACHE_DIR, 'static-evidence-cache-v8.json');
const IMAGE_REPORT_FILE = path.join(CACHE_DIR, 'google-image-url-report-v8.json');
const EVIDENCE_REPORT_FILE = path.join(CACHE_DIR, 'static-evidence-report-v8.json');
const CATEGORY_REPORT_FILE = path.join(CACHE_DIR, 'category-seo-report-v8.json');
const PART_NUMBER_REPORT_FILE = path.join(CACHE_DIR, 'part-number-report-v8.json');
const PRODUCT_PREVIEW_FILE = path.join(CACHE_DIR, 'product-preview-v8.json');
...
type DiscoveryPageOutcome = {
  product: DiscoveredProduct | null;
  reason?: string;
};

type DiscoveryScanCache = Record<string, {
...
// V5 được giữ làm nguồn catalog tĩnh tham chiếu; V8 ghép thêm model tĩnh để
// đạt đúng quota 850 và không gọi crawler khám phá sản phẩm trong main().
...
// Mở rộng nguồn chính hãng cho 54 danh mục V7. Domain là allow-list; crawler
...
type StaticSeedProduct = { name: string; partNumber: string };

function staticNamed(name: string, partNumber: string): StaticSeedProduct {
  return { name, partNumber };
}

function staticModels(brand: string, models: string[]): StaticSeedProduct[] {
  return models.map((partNumber) => ({ name: `${brand} ${partNumber}`, partNumber }));
}

// Danh sách bổ sung tĩnh cho đủ quota 850. Mỗi dòng khai báo part number/model
// thật dùng làm code/sku/mpn; script không sinh mã CTC hoặc biến thể model giả.
const STATIC_PRODUCT_ADDITIONS: Record<string, StaticSeedProduct[]> = {
  'can-bang-tai': [
    staticNamed('Fortinet FortiGate FG-40F', 'FG-40F'),
    staticNamed('Fortinet FortiGate FG-80F', 'FG-80F'),
    staticNamed('Fortinet FortiGate FG-100F', 'FG-100F'),
    staticNamed('Peplink Balance 380X', 'Balance 380X'),
    staticNamed('DrayTek Vigor3912 Multi-WAN Router', 'Vigor3912'),
    staticNamed('Fortinet FortiGate FG-200F', 'FG-200F'),
    staticNamed('Peplink Balance 580X', 'Balance 580X'),
  ],
  switch: [
    staticNamed('Ubiquiti UniFi USW-Enterprise-24-PoE', 'USW-Enterprise-24-PoE'),
  ],
  'odf-tu-phan-phoi-quang': [
    staticNamed('CommScope FIST-GPS2 Fiber Patch Shelf', 'FIST-GPS2'),
    staticNamed('Corning EDGE-CP48 Fiber Enclosure', 'EDGE-CP48'),
    staticNamed('CommScope FIST-GPS2-2HU Fiber Shelf', 'FIST-GPS2-2HU'),
    staticNamed('CommScope FIST-GPS2-3HU Fiber Shelf', 'FIST-GPS2-3HU'),
    staticNamed('Corning EDGE-CP24-E3 Fiber Housing', 'EDGE-CP24-E3'),
    staticNamed('Corning EDGE-CP48-E3 Fiber Housing', 'EDGE-CP48-E3'),
    staticNamed('Panduit FCE1U Fiber Enclosure', 'FCE1U'),
    staticNamed('Panduit FCE2U Fiber Enclosure', 'FCE2U'),
  ],
  'ip-pbx-tong-dai': [
    staticNamed('Yeastar P570 IP PBX', 'P570'),
    staticNamed('Sangoma PBXact 75 IP PBX', 'PBXact 75'),
  ],
  'dien-thoai-ip': [
    ...staticModels('Yealink', ['SIP-T58W-Pro', 'SIP-T31W']),
    ...staticModels('Fanvil', ['V65', 'V67']),
    ...staticModels('Grandstream', ['GRP2670']),
    ...staticModels('Cisco', ['CP-8875-K9']),
  ],
  'cap-mang': [
    staticNamed('Belden DataTuff 7934A Industrial Ethernet Cable', '7934A'),
    staticNamed('Belden DataTuff 7929A Cat6 Cable', '7929A'),
    staticNamed('Panduit PUP6004BU-UY Cat6 UTP Cable', 'PUP6004BU-UY'),
    staticNamed('CommScope 884024508/10 Cat6 UTP Cable', '884024508/10'),
    staticNamed('CommScope 884024614/10 Cat6 UTP Cable', '884024614/10'),
    staticNamed('Panduit PUP6C04BU-U Cat6 Cable', 'PUP6C04BU-U'),
    staticNamed('Panduit PFL6X04BU-CEG Cat6A Cable', 'PFL6X04BU-CEG'),
    staticNamed('Belden 2412 Cat6 Cable', '2412'),
    staticNamed('Belden 2413 Cat6 Cable', '2413'),
    staticNamed('Belden 10GX12 Cat6A Cable', '10GX12'),
    staticNamed('Dintek 1101-03003 Cat6 UTP Cable', '1101-03003'),
  ],
  'cap-quang': [
    staticNamed('Corning 012E8F-31131-29 Fiber Cable', '012E8F-31131-29'),
    staticNamed('Corning 024E8F-31131-29 Fiber Cable', '024E8F-31131-29'),
    staticNamed('LS FOC-SM-12C-G652D Fiber Cable', 'FOC-SM-12C-G652D'),
    staticNamed('Vinacap FOC-ADSS-24FO Fiber Cable', 'FOC-ADSS-24FO'),
    staticNamed('Corning 008EUC-T4101D20 Fiber Cable', '008EUC-T4101D20'),
    staticNamed('CommScope P-012-LA-8W-F12NS Fiber Cable', 'P-012-LA-8W-F12NS'),
    staticNamed('LS FOC-MM-08C-OM3 Fiber Cable', 'FOC-MM-08C-OM3'),
  ],
  'patch-panel': [
    staticNamed('TP-Link TL-PP110U 24-Port Patch Panel', 'TL-PP110U'),
  ],
  'pc-may-tinh-de-ban': [
    staticNamed('Dell OptiPlex 7020 Small Form Factor', 'OptiPlex 7020'),
    staticNamed('HP Elite SFF 800 G9 Desktop PC', 'Elite SFF 800 G9'),
    staticNamed('Lenovo ThinkCentre M90s Gen 5', 'M90s Gen 5'),
    staticNamed('ASUS ExpertCenter D500ME', 'D500ME'),
    staticNamed('Acer Veriton X2710G', 'X2710G'),
    staticNamed('Dell OptiPlex 7020 Tower Plus', 'OptiPlex 7020 Tower Plus'),
  ],
  'mini-pc': [
    staticNamed('ASUS NUC 14 Pro NUC14RVH', 'NUC14RVH'),
    staticNamed('Gigabyte BRIX GB-BER7-8840', 'GB-BER7-8840'),
    staticNamed('Minisforum UM790 Pro', 'UM790 Pro'),
    staticNamed('Beelink SER8 Mini PC', 'SER8'),
    staticNamed('Zotac ZBOX CI337 nano', 'CI337 nano'),
    staticNamed('Intel NUC 13 Pro NUC13ANHi7', 'NUC13ANHi7'),
    staticNamed('Gigabyte BRIX GB-BEi5-1240', 'GB-BEi5-1240'),
  ],
  laptop: [
    staticNamed('Dell Latitude 3450 Laptop', 'Latitude 3450'),
    staticNamed('Dell Latitude 5450 Laptop', 'Latitude 5450'),
    staticNamed('Dell Latitude 7450 Laptop', 'Latitude 7450'),
    staticNamed('Dell Latitude 9450 2-in-1', 'Latitude 9450'),
    staticNamed('Dell Precision 3591 Mobile Workstation', 'Precision 3591'),
    staticNamed('HP EliteBook 640 G11', 'EliteBook 640 G11'),
    staticNamed('HP EliteBook 840 G11', 'EliteBook 840 G11'),
    staticNamed('HP EliteBook 1040 G11', 'EliteBook 1040 G11'),
    staticNamed('HP ProBook 440 G11', 'ProBook 440 G11'),
    staticNamed('HP ZBook Power 16 G11', 'ZBook Power 16 G11'),
    staticNamed('Lenovo ThinkPad E14 Gen 6', 'ThinkPad E14 Gen 6'),
    staticNamed('Lenovo ThinkPad T14 Gen 5', 'ThinkPad T14 Gen 5'),
    staticNamed('Lenovo ThinkPad X1 Carbon Gen 12', 'ThinkPad X1 Carbon Gen 12'),
    staticNamed('Lenovo ThinkPad L14 Gen 5', 'ThinkPad L14 Gen 5'),
    staticNamed('Lenovo ThinkBook 14 G7', 'ThinkBook 14 G7'),
    staticNamed('ASUS ExpertBook B1403CVA', 'B1403CVA'),
    staticNamed('ASUS ExpertBook B1503CVA', 'B1503CVA'),
    staticNamed('ASUS ExpertBook B5404CMA', 'B5404CMA'),
    staticNamed('ASUS Zenbook UX3405MA', 'UX3405MA'),
    staticNamed('Acer TravelMate P2 TMP214-55', 'TMP214-55'),
    staticNamed('Acer TravelMate P4 TMP414-53', 'TMP414-53'),
    staticNamed('Apple MacBook Air A3113', 'A3113'),
    staticNamed('Apple MacBook Pro A2992', 'A2992'),
    staticNamed('MSI Modern 14 C13M', 'Modern 14 C13M'),
    staticNamed('MSI Prestige 14 AI Evo C1MG', 'C1MG'),
    staticNamed('Microsoft Surface Laptop 6 ZGJ-00001', 'ZGJ-00001'),
    staticNamed('Microsoft Surface Pro 10 ZDT-00001', 'ZDT-00001'),
    staticNamed('HP ZBook Fury 16 G11 A8ZS4AV', 'A8ZS4AV'),
  ],
  'may-tram-workstation': [
    staticNamed('Dell Precision 3680 Tower Workstation', 'Precision 3680'),
    staticNamed('Dell Precision 5860 Tower Workstation', 'Precision 5860'),
    staticNamed('Dell Precision 7875 Tower Workstation', 'Precision 7875'),
    staticNamed('Dell Precision 5690 Mobile Workstation', 'Precision 5690'),
    staticNamed('HP Z2 G9 Tower Workstation', 'Z2 G9'),
    staticNamed('HP Z4 G5 Workstation', 'Z4 G5'),
    staticNamed('HP Z6 G5 A Workstation', 'Z6 G5 A'),
    staticNamed('HP Z8 Fury G5 Workstation', 'Z8 Fury G5'),
    staticNamed('Lenovo ThinkStation P3 Tower', 'ThinkStation P3'),
    staticNamed('Lenovo ThinkStation P5', 'ThinkStation P5'),
    staticNamed('Lenovo ThinkStation P7', 'ThinkStation P7'),
    staticNamed('ASUS ProArt Station PD500TE', 'PD500TE'),
  ],
  'may-tinh-all-in-one': [
    staticNamed('Dell OptiPlex 7420 All-in-One', 'OptiPlex 7420'),
    staticNamed('Dell OptiPlex 7410 All-in-One', 'OptiPlex 7410'),
    staticNamed('HP ProOne 440 G9 All-in-One', 'ProOne 440 G9'),
    staticNamed('HP ProOne 600 G6 All-in-One', 'ProOne 600 G6'),
    staticNamed('HP EliteOne 870 G9 All-in-One', 'EliteOne 870 G9'),
    staticNamed('Lenovo ThinkCentre neo 50a 24 Gen 5', 'neo 50a 24 Gen 5'),
    staticNamed('Lenovo ThinkCentre M90a Pro Gen 4', 'M90a Pro Gen 4'),
    staticNamed('Lenovo ThinkCentre M70a Gen 5', 'M70a Gen 5'),
    staticNamed('ASUS ExpertCenter E5402WHAK', 'E5402WHAK'),
    staticNamed('ASUS A5702WVA All-in-One', 'A5702WVA'),
    staticNamed('Acer Aspire C24-1800', 'C24-1800'),
    staticNamed('Acer Veriton Z4 VZ4717GT', 'VZ4717GT'),
  ],
  'man-hinh-may-tinh': [
    ...staticModels('Dell', ['P2425H', 'P2725H', 'U2724D', 'U3425WE']),
    staticNamed('HP Series 5 Pro 524pf', '524pf'),
    staticNamed('HP Series 5 Pro 527pq', '527pq'),
    ...staticModels('Lenovo ThinkVision', ['T24i-30', 'P27h-30']),
    ...staticModels('LG', ['27BA850-B', '34WQ75C-B']),
    ...staticModels('Samsung', ['LS27D600EAEXXV', 'LS32D800UAEXXV']),
    ...staticModels('ASUS ProArt', ['PA278CGV']),
    ...staticModels('ASUS', ['VA27EHF']),
    ...staticModels('ViewSonic', ['VG2709-2K-MHD', 'VP2756-4K']),
    ...staticModels('BenQ', ['PD2705Q']),
    staticNamed('Màn hình di động cảm ứng VSP VP1560FST1 (15.6 inch/FHD/IPS/62Hz/5ms/loa)', 'VP1560FST1'),
  ],
  'may-in-thiet-bi-in': [
    ...staticModels('HP LaserJet', ['4003dn', 'MFP-4103fdw', 'Enterprise-M507dn']),
    ...staticModels('Canon', ['LBP246dw', 'MF465dw', 'imageRUNNER-2425i']),
    ...staticModels('Brother', ['HL-L6415DW', 'MFC-L6915DW', 'DCP-L5510DN']),
    ...staticModels('Epson', ['L6490', 'L15150', 'WF-C5890']),
    ...staticModels('Fujifilm', ['Apeos-2150-NDA', 'Apeos-C325z', 'DocuPrint-P375dw']),
    ...staticModels('Ricoh', ['M-320F', 'P-311', 'IM-C2010']),
  ],
  'may-in-nhan': [
    staticNamed('TSC TE210 Label Printer', 'TE210'),
    staticNamed('Godex G500 Label Printer', 'G500'),
    staticNamed('Honeywell PM45A Label Printer', 'PM45A'),
  ],
  'may-quet-ma-vach': [
    ...staticModels('Zebra', ['DS2208', 'DS4608-SR', 'DS9308']),
    ...staticModels('Honeywell', ['Voyager-1470g', 'Xenon-XP-1950g', 'Granit-1990i']),
    ...staticModels('Datalogic', ['Gryphon-GD4520', 'Magellan-1500i']),
    ...staticModels('Newland', ['HR32-Marlin', 'FR4080-Koi-II']),
    ...staticModels('Opticon', ['OPN-2006', 'L-51X']),
  ],
  'thiet-bi-pos': [
    ...staticModels('Sunmi', ['T2s', 'T2-Mini', 'D3-Pro']),
    ...staticModels('Posiflex', ['PS-3616G2', 'RT-6015', 'XT-3815']),
    ...staticModels('Epson', ['TM-m30III', 'TM-T88VII']),
    ...staticModels('HP', ['Engage-One-Pro', 'Engage-Flex-Pro-C']),
    ...staticModels('Zebra', ['TC22', 'TC27']),
  ],
  'kiosk-tu-phuc-vu': [
    staticNamed('Elo EloPOS Z70 Self-Service Kiosk', 'EloPOS Z70'),
    staticNamed('Elo I-Series 3 Self-Service Kiosk', 'I-Series 3'),
    staticNamed('Samsung KM24C-3 Kiosk', 'KM24C-3'),
    staticNamed('LG 27KC3PK-C Kiosk', '27KC3PK-C'),
    staticNamed('Advantech UTC-515H Touch Kiosk', 'UTC-515H'),
    staticNamed('Posiflex TK-3250 Kiosk', 'TK-3250'),
    staticNamed('NCR CX3 Self-Service Kiosk', 'CX3'),
    staticNamed('AOPEN eTILE-X19 Kiosk', 'eTILE-X19'),
    staticNamed('NCR SelfServ 27 Kiosk', 'SelfServ 27'),
    staticNamed('Diebold Nixdorf DN Series EASY ONE', 'DN EASY ONE'),
    staticNamed('Pyramid polytouch 32 passport Kiosk', 'polytouch 32 passport'),
  ],
  'may-tinh-cong-nghiep': [
    ...staticModels('Advantech', ['UNO-2484G', 'ARK-3534', 'MIC-770-V3']),
    ...staticModels('Siemens SIMATIC', ['IPC127E', 'IPC427E']),
    ...staticModels('Beckhoff', ['C6030', 'CX5140']),
    ...staticModels('Axiomtek', ['eBOX671-521-FL']),
    ...staticModels('OnLogic', ['Karbon-410']),
    ...staticModels('ASUS IoT', ['PE200U']),
    ...staticModels('Cincoze', ['DS-1402', 'P2202E']),
  ],
  'thiet-bi-luu-tru': [
    ...staticModels('Synology', ['DS923+', 'DS1823xs+', 'RS2423+']),
    ...staticModels('QNAP', ['TS-464', 'TVS-h674', 'TS-873A']),
    ...staticModels('Dell EMC PowerVault', ['ME5024', 'ME5012']),
    ...staticModels('HPE', ['MSA-2060', 'Alletra-5010']),
    ...staticModels('Lenovo', ['DE4000H', 'DM5000H']),
    ...staticModels('Western Digital', ['My-Cloud-EX2-Ultra', 'Ultrastar-Data60']),
    ...staticModels('Seagate', ['Exos-X-5U84', 'Nytro-3332']),
    ...staticModels('Kingston', ['DC600M']),
    ...staticModels('Samsung', ['PM893']),
  ],
  'ups-thiet-bi-data-center': [
    ...staticModels('APC', ['SRT3000XLI', 'SRT6000XLI', 'SMT3000RMI2U']),
    ...staticModels('Eaton', ['9PX3000IRT2U', '9PX6Ki', '93E-15KiVA']),
    ...staticModels('Vertiv', ['GXT5-3000IRT2UXL', 'EXS-20kVA', 'ITA2-10kVA']),
    ...staticModels('Santak', ['C3K', 'C6K', '3C10KS']),
    ...staticModels('Delta', ['RT-3K', 'Ultron-HPH-20K']),
    ...staticModels('Socomec', ['NETYS-RT-3300', 'MODULYS-GP-25']),
    ...staticModels('Schneider Electric', ['Galaxy-VS-20kW', 'Easy-UPS-3S-20kVA']),
  ],
  'tu-rack-phu-kien': [
    ...staticModels('APC NetShelter', ['AR3100', 'AR3300', 'AR3350']),
    ...staticModels('CommScope', ['VCM-DS-84B']),
    ...staticModels('Schneider Electric', ['NSYSF20860']),
    ...staticModels('ECP', ['ECP42U-D1000']),
    ...staticModels('Dintek', ['19-6842']),
    ...staticModels('Alantek', ['42U600']),
    ...staticModels('Unirack', ['UR42-1000']),
    ...staticModels('Panduit', ['R4P23']),
  ],
  'camera-ip': [
    ...staticModels('KBVISION', ['KX-A2014N3', 'KX-A2111N3', 'KX-C2011N3', 'KX-C4201N', 'KX-EAi4305MN2']),
    ...staticModels('Hikvision', ['DS-2CD1023G2-LIU', 'DS-2CD1143G2-LIU', 'DS-2CD2046G2-IU', 'DS-2CD2146G2-ISU', 'DS-2CD2T47G2-L']),
    ...staticModels('Dahua', ['DH-IPC-HFW1239S1-LED-S5', 'DH-IPC-HDBW2239E-S-IL', 'DH-IPC-HFW2449S-S-IL', 'DH-IPC-HDW3441T-ZS-S2']),
    ...staticModels('Uniview', ['IPC2124LB-SF28-A', 'IPC3614LE-ADF28K-G', 'IPC2C22LE-SF40-A']),
    ...staticModels('Hanwha Vision', ['QNO-6082R', 'QND-6082R']),
    ...staticModels('Bosch', ['NBE-5704-AL']),
  ],
  'camera-analog': [
    ...staticModels('KBVISION', ['KX-AF2112L-A', 'KX-AD2111C-A']),
    ...staticModels('Hikvision', ['DS-2CE16D0T-LPFS', 'DS-2CE76D0T-LPFS']),
    ...staticModels('Dahua', ['DH-HAC-HFW1209CP-LED-S5', 'DH-HAC-HDW1200TLP-A']),
    ...staticModels('Uniview', ['UAC-T112-F28', 'UAC-B112-F40']),
  ],
  'camera-ptz': [
    ...staticModels('KBVISION', ['KX-DDAi4329ZPN3']),
    ...staticModels('Hikvision', ['DS-2DE4425IW-DE']),
    ...staticModels('Dahua', ['DH-SD5A432GB-HNR']),
    ...staticModels('Uniview', ['IPC6854SR-X44U']),
    ...staticModels('Hanwha Vision', ['XNP-6400RW']),
    ...staticModels('Bosch', ['NDP-5512-Z30']),
  ],
  'camera-wifi': [
    ...staticModels('KBVISION', ['KX-A21F']),
    ...staticModels('Hikvision', ['DS-2CV2121G2-IDW']),
    ...staticModels('Dahua', ['DH-IPC-HFW1230DS1-SAW']),
    ...staticModels('Uniview', ['IPC6412LR-X5P']),
    ...staticModels('EZVIZ', ['H8c-3MP']),
    ...staticModels('Imou', ['IPC-S7XP-6M0WED']),
  ],
  'camera-ai': [
    ...staticModels('KBVISION', ['KX-CAiF4005MN2-TiF-A', 'KX-DAi5005MN-EAB']),
    ...staticModels('Hikvision', ['DS-2CD7A26G0-IZHSY', 'DS-2CD2387G2H-LISU-SL']),
    ...staticModels('Dahua', ['DH-IPC-HFW5442E-ZE', 'DH-IPC-HDBW5842H-ZHE']),
    ...staticModels('Hanwha Vision', ['PNO-A9081R']),
    ...staticModels('Bosch', ['NBE-7604-AL']),
  ],
  'dau-ghi-hinh-nvr': [
    ...staticModels('KBVISION', ['KX-E4K824256N4', 'KX-DAi4K8432EN3', 'KX-A4K8116N3']),
    ...staticModels('Hikvision', ['DS-7616NI-K2', 'DS-7732NXI-K4']),
    ...staticModels('Dahua', ['DHI-NVR5216-EI', 'DHI-NVR608H-64-XI']),
    ...staticModels('Uniview', ['NVR301-16S3']),
  ],
  'dau-ghi-hinh-dvr-xvr': [
    ...staticModels('KBVISION', ['KX-DAi8116H3', 'KX-7108T']),
    ...staticModels('Hikvision', ['iDS-7216HQHI-M1-S']),
    ...staticModels('Dahua', ['XVR5116HS-I3']),
    ...staticModels('Uniview', ['XVR301-08G3']),
  ],
  'phu-kien-camera': [
    ...staticModels('Hikvision', ['DS-1280ZJ-DM21']),
    ...staticModels('Dahua', ['PFA130-E']),
    ...staticModels('KBVISION', ['KX-BF01']),
    ...staticModels('Uniview', ['TR-JB05-A-IN']),
  ],
  'thiet-bi-hoi-nghi-truyen-hinh': [
    staticNamed('Logitech Rally Bar', 'Rally Bar'),
    staticNamed('Logitech Rally Bar Mini', 'Rally Bar Mini'),
    staticNamed('Logitech MeetUp 2', 'MeetUp 2'),
    staticNamed('Yealink MeetingBar A30', 'MeetingBar A30'),
    staticNamed('Yealink MeetingBar A40', 'MeetingBar A40'),
    staticNamed('Poly Studio X52', 'Studio X52'),
    staticNamed('Cisco Room Bar', 'Room Bar'),
    staticNamed('Jabra PanaCast 50', 'PanaCast 50'),
  ],
  'may-chieu-thiet-bi-trinh-chieu': [
    ...staticModels('Epson', ['EB-L260F', 'EB-992F']),
    ...staticModels('BenQ', ['EH600', 'LH730']),
    ...staticModels('ViewSonic', ['LS740HD']),
    ...staticModels('Panasonic Connect', ['PT-VMZ71']),
    ...staticModels('Sony', ['VPL-PHZ61']),
    ...staticModels('Optoma', ['ZH450ST']),
  ],
  'thiet-bi-ngoai-vi': [
    staticNamed('Logitech MX Keys S 920-011565', '920-011565'),
    staticNamed('Logitech MX Master 3S 910-006561', '910-006561'),
    staticNamed('Logitech Brio 500 960-001422', '960-001422'),
    staticNamed('Dell KB216 580-ADMT', '580-ADMT'),
    staticNamed('Dell MS3320W 570-ABDU', '570-ABDU'),
    staticNamed('HP 320K Keyboard 9SR37AA', '9SR37AA'),
    staticNamed('HP 435 Multi-Device Mouse 3B4Q5AA', '3B4Q5AA'),
    staticNamed('Lenovo Professional Wireless Keyboard 4X30H56886', '4X30H56886'),
    staticNamed('Microsoft Modern Mobile Mouse KTF-00013', 'KTF-00013'),
    staticNamed('Rapoo MT750S Mouse', 'MT750S'),
    staticNamed('Rapoo VT9PRO Mouse', 'VT9PRO'),
    staticNamed('Kingston Workflow Station WFS-U', 'WFS-U'),
  ],
  'inverter-hybrid': [
    ...staticModels('Deye', ['SUN-5K-SG03LP1-EU', 'SUN-6K-SG03LP1-EU', 'SUN-8K-SG01LP1-EU', 'SUN-10K-SG04LP3-EU', 'SUN-12K-SG04LP3-EU']),
    ...staticModels('Huawei', ['SUN2000-5K-MAP0', 'SUN2000-6K-MAP0', 'SUN2000-8K-MAP0', 'SUN2000-10K-MAP0', 'SUN2000-12K-MAP0']),
    ...staticModels('Sungrow', ['SH5.0RS', 'SH6.0RS', 'SH10RT', 'SH15T', 'SH20T']),
    ...staticModels('Growatt', ['SPH5000TL-BL-UP', 'SPH6000TL3-BH-UP', 'MOD10KTL3-XH', 'WIT-50KTL3-A-LV']),
    ...staticModels('GoodWe', ['GW5000-ES-20', 'GW10K-ET-20', 'GW15K-ET', 'GW20K-ET']),
    ...staticModels('Solis', ['S6-EH1P6K-L-PRO', 'S6-EH3P12K-H']),
  ],
  'bo-toi-uu-cong-suat-optimizer': [
    ...staticModels('Huawei', ['SUN2000-450W-P2', 'SUN2000-600W-P']),
    ...staticModels('SolarEdge', ['S440', 'S500', 'S650B', 'P850']),
    ...staticModels('Tigo', ['TS4-A-O', 'TS4-X-O', 'TS4-A-S']),
    ...staticModels('SMA', ['TS4-R-O']),
  ],
  'tu-dien-nang-luong-mat-troi': [
    ...staticModels('Schneider Electric', ['NSYS3D5420P', 'LVS08204']),
    ...staticModels('ABB', ['1SL0211A00', '1SLM004100A1204']),
    ...staticModels('LS Electric', ['ABN103c', 'ABS103c']),
    ...staticModels('Siemens', ['8GK1322-5KK12', '8MF1006-2GS30']),
    ...staticModels('Chint', ['NXDB-8', 'NXDB-12']),
    ...staticModels('Mitsubishi Electric', ['BCH-A-12', 'BCH-A-16']),
  ],
  'thiet-bi-bao-ve-dien-mat-troi': [
    ...staticModels('Schneider Electric', ['A9L40271', 'A9F74220', 'C11H-DC']),
    ...staticModels('ABB', ['OVR-PV-T2-40-1000-P-TS', 'S802PV-S16']),
    ...staticModels('Siemens', ['5SD7483-0', '5SY4106-7']),
    ...staticModels('LS Electric', ['BKN-b-DC']),
    ...staticModels('Chint', ['NB1-63DC', 'NXU-II-G']),
    ...staticModels('Phoenix Contact', ['VAL-MS-T1-T2-1000DC-PV-2+V', 'SOL-SC-1ST-0-DC-2MPPT-1001']),
    ...staticModels('DEHN', ['DG-M-YPV-SCI-1000-FM', 'DG-M-YPV-1200-FM', 'DEHNcube-YPV-SCI-1000-1M']),
  ],
  'cap-dau-noi-solar': [
    ...staticModels('Stäubli', ['PV-KST4-6II-UR', 'PV-KBT4-6II-UR', '32.0017P0001-UR']),
    ...staticModels('LAPP', ['OLFLEX-SOLAR-XLWP-1500V', 'H1Z2Z2-K-4']),
    ...staticModels('HELUKABEL', ['SOLARFLEX-X-H1Z2Z2-K-4', 'SOLARFLEX-X-H1Z2Z2-K-6']),
    ...staticModels('CADIVI', ['CVV-SOLAR-4', 'CVV-SOLAR-6']),
    ...staticModels('LS', ['H1Z2Z2-K-4', 'H1Z2Z2-K-6']),
    ...staticModels('Leader Solar', ['TUV-EN50618-4', 'PV-LC4']),
    ...staticModels('Amphenol', ['H4-Plus-PV-100508-F', 'H4-UTX-PV-100548']),
  ],
  'he-khung-gia-do-solar': [
    ...staticModels('Schletter', ['FixGrid-18', 'ProLine-RoofHook-SL']),
    ...staticModels('Clenergy', ['PV-ezRack-SolarRoof-Pro-2.0', 'PV-ezRack-ComT']),
    ...staticModels('K2 Systems', ['SingleRail-36', 'D-Dome-6.10']),
    ...staticModels('Antaisolar', ['Helios-Roof', 'Tai-Simple']),
    ...staticModels('Kseng', ['KST-1', 'KTR-2']),
  ],
  'thiet-bi-giam-sat-do-dem': [
    ...staticModels('Huawei', ['SmartLogger3000A', 'SmartLogger3000B']),
    ...staticModels('Sungrow', ['COM100E', 'Logger1000B']),
    ...staticModels('SolarEdge', ['SE1000-S1-MTR']),
    ...staticModels('SMA', ['EDMM-10']),
    ...staticModels('Schneider Electric', ['EGX150']),
    ...staticModels('Carlo Gavazzi', ['VMU-C-EM']),
  ],
  'cong-to-thiet-bi-do-dien': [
    ...staticModels('Schneider Electric', ['PM5560']),
    ...staticModels('ABB', ['M4M-30']),
    ...staticModels('Siemens', ['SENTRON-PAC3220']),
    ...staticModels('Janitza', ['UMG-96RM-E']),
    ...staticModels('Socomec', ['DIRIS-A-40']),
  ],
  'thiet-bi-ve-sinh-tam-pin': [
    ...staticModels('Kärcher', ['iSolar-400', 'iSolar-800']),
    ...staticModels('Ecoppia', ['E4']),
    ...staticModels('SolarCleano', ['F1A']),
    ...staticModels('SunBrush', ['mobil-Compact']),
  ],
  'ac-quy-lithium-lifepo4': [
    ...staticModels('Pylontech', ['US2000C', 'UP5000', 'Force-L2']),
    ...staticModels('BYD', ['Battery-Box-HVS-5.1']),
    ...staticModels('Huawei', ['LUNA2000-5-E0']),
    ...staticModels('Deye', ['SE-G5.1Pro-B']),
    ...staticModels('Dyness', ['DL5.0C']),
    ...staticModels('Growatt', ['APX-5.0P-B1']),
    ...staticModels('Sungrow', ['SBR096']),
    ...staticModels('Narada', ['NPFC100']),
    ...staticModels('Pylontech', ['Pelio-L-5.12']),
    ...staticModels('BYD', ['Battery-Box-LVL-15.4']),
  ],
  'ac-quy-chi-vrla': [
    ...staticModels('CSB', ['GP1272', 'GP12120', 'HRL1234W']),
    ...staticModels('Yuasa', ['NP7-12', 'NP12-12']),
    ...staticModels('Panasonic', ['LC-R127R2PG']),
    ...staticModels('Leoch', ['DJM12-100', 'LP12-65']),
    ...staticModels('Fiamm', ['12FGH65']),
    ...staticModels('Exide', ['EP65-12']),
    ...staticModels('Rocket', ['ES12-100']),
    ...staticModels('Vision', ['CP12170', '6FM100']),
    ...staticModels('Narada', ['6-GFM-100', 'MPG12V100']),
  ],
  'ac-quy-nuoc-traction': [
    ...staticModels('Trojan', ['T-105', 'T-125', 'J305H-AC']),
    ...staticModels('Hoppecke', ['5-PzS-625']),
    ...staticModels('EnerSys', ['12-125-13']),
    ...staticModels('TAB', ['6PzS750']),
    ...staticModels('Crown', ['CR-235']),
    ...staticModels('Rolls', ['S-550']),
    ...staticModels('GS Yuasa', ['EB-100']),
    ...staticModels('Exide', ['Classic-FT12-125']),
    ...staticModels('EnerSys', ['12V-5PzS-625', '24V-4PzS-500']),
    ...staticModels('Hoppecke', ['trak-air-5PzS625', 'trak-basic-6PzS750']),
    ...staticModels('TAB', ['8PzS1000']),
  ],
};

const STATIC_EXCLUDED_PRODUCT_NAMES = new Set([
  'Ubiquiti UniFi Enterprise 24 PoE',
  'DrayTek Vigor 3910 10G Load Balancer',
  'DrayTek Vigor 2962 Dual WAN Balancer',
  'Ubiquiti UFiber SFP+ 10G SM 10km',
  'CommScope ODF 24FO LC/UPC Rack 1U',
  'CommScope ODF 48FO LC/APC Rack 2U',
  'CommScope ODF 96FO SC/UPC Rack 4U',
  'FiberNet ODF 24FO SC/APC',
  'FiberNet ODF 48FO LC/UPC 2U Rack',
  'CommScope ODF 144FO LC/APC Rack 6U',
  'CommScope Cat5e 305m UTP PVC',
  'Belden DataTwist Cat6 UTP 305m',
  'AMP Netconnect Cat6A STP 305m',
  'CommScope Cat6A 10G ANEXT+',
  'LS Cable Cat6 UTP 305m Vietnam Made',
  'CommScope Cat7 S/FTP 305m',
  'Corning Cat6A F/UTP 305m Reel',
  'CommScope OS2 SM G652D 12FO 1km',
  'CommScope ADSS 24FO G652D Span 100m',
  'CommScope OPGW 24FO Overhead Cable 1km',
  'Corning ClearCurve OM4 12FO',
  'Draka BKT OS2 48FO Single-mode',
  'OFS AllWave 96FO SM G652D',
  'CommScope 4FO SM OS2 Indoor Armored',
  'CommScope 8FO OM3 MMF Indoor Plenum',
  'CommScope Pigtail LC/UPC SM OS2 1m',
  'CommScope 12FO G652D Outdoor Direct Burial',
  'CommScope GigaSPEED X10D 24-port Cat6A',
  'CommScope Cat6A Shielded RJ45 Field Plug',
  'CommScope Cat6 UTP Patch Cord 1m',
  'CommScope Cat6 UTP Patch Cord 3m',
  'CommScope Cat6A S/FTP Patch Cord 2m',
  ...
  'CommScope Fiber Optic Splice Tray 24 Core',
  'Dintek Cat6 UTP Patch Cord 3m',
  'HPE Synergy 480 Gen10 Blade',
  'HP Elite Tower 800 G9 Desktop PC',
  'Intel NUC 13 Pro i7-1360P Arena Canyon',
  'AAEON UP Squared Pro i5-1135G7 Industrial',
  'Microsoft Surface Laptop 5 i7 13.5"',
  'Microsoft Surface Pro 9 i7 13" Tablet',
  'HP ZBook Fury 16 G9 i9 Workstation',
  'Datamax H-Class Mark II Industrial',
  'NCR SelfServ 90 Kiosk',
  'Diebold Nixdorf DN Series EASY ONE Kiosk',
  'Pyramid polytouch 32 Curve Kiosk',
  'REC Alpha Pure-RX 410W',
  'REC Alpha Pro M 430W Series',
  'SunPower Maxeon 6 AC Module 420W',
  'Vikram Solar Somera 380W PERC',
  'Waaree Energies 535W Bifacial PERC',
  'LONGI Hi-MO X LR5-72HGD 600W',
  'Fronius Symo Gen24 Plus 8.0kW',
  'SMA Sunny Tripower X 25',
  'Deye SUN-6K-SG03LP1-EU 6kW Hybrid',
  'Deye SUN-12K-SG04LP3 12kW Hybrid 3P',
  'Goodwe GW5048D-ES 5kW Hybrid Offgrid',
  'EG4 LL-S Lithium Battery 48V 100Ah',
  'BYD Battery-Box Premium LVL 15.4',
  'EnerSys Marathon L 2V 1000Ah Traction',
  'EnerSys PowerSafe 2V 600Ah OPzS',
  'Hoppecke OPzS 2V 800Ah Flooded',
  'Hoppecke Grid Power S 2V 1500Ah',
  'Fiamm 2 SLA M 400 2V 400Ah',
]);

function buildStaticProductCatalog(): CatalogGroup[] {
  const referenceBySlug = new Map(V5_REFERENCE_PRODUCT_CATALOG.map((group) => [group.slug, group]));
  return CATEGORY_TARGETS.map((targetItem) => {
    const reference = referenceBySlug.get(targetItem.slug);
    const rows: StaticSeedProduct[] = [
      ...(reference?.products || []).map((name) => ({ name, partNumber: PART_NUMBER_OVERRIDES[name] || '' })),
      ...(STATIC_PRODUCT_ADDITIONS[targetItem.slug] || []),
    ];
    const unique = new Map<string, StaticSeedProduct>();
    for (const row of rows) {
      if (STATIC_EXCLUDED_PRODUCT_NAMES.has(row.name)) continue;
      const key = slugify(row.name);
      if (!key || unique.has(key)) continue;
      unique.set(key, row);
      if (row.partNumber) PART_NUMBER_OVERRIDES[row.name] = row.partNumber;
    }
    const products = [...unique.values()].slice(0, targetItem.quota).map((row) => row.name);
    if (products.length !== targetItem.quota) {
      throw new Error(
        `Static catalog thiếu ${targetItem.category}: cần ${targetItem.quota}, hiện có ${products.length}. ` +
        'Không sinh model giả để bù quota.',
      );
    }
    return {
      ...
      brand: targetItem.brands.join(' / '),
      imageQuery: `${targetItem.category} official product image`,
      fallbackKey: 'default',
      quota: targetItem.quota,
      products,
    };
  });
}
...
function assertCatalog(): void {
  ...
  if (products.length < MIN_TOTAL_PRODUCTS || products.length > MAX_TOTAL_PRODUCTS) {
    throw new Error(
      `Catalog V8 phải nằm trong ${MIN_TOTAL_PRODUCTS}–${MAX_TOTAL_PRODUCTS} sản phẩm, hiện có ${products.length}. ` +
...
export async function resolveProductImage(productName: string): Promise<VerifiedImage> {
  ...
  if (!SERPER_ENABLED) {
    throw new Error(
      `Ảnh trực tiếp trên trang hãng không đạt kiểm tra cho ${productName}. ` +
      'Free Mode không dùng ảnh giả; hãy bổ sung URL ảnh chính hãng vào cache/manifest hoặc bật ENABLE_SERPER=true.',
    );
  }
...
async function searchGoogleWeb(
  ...
  try {
    ...
    const response = await fetch('https://google.serper.dev/search', {
      ...
      body: JSON.stringify({
        q,
        gl: locale.gl || DISCOVERY_GL,
        hl: locale.hl || DISCOVERY_HL,
        num: Math.min(20, Math.max(1, resultCount)),
    ...
    return (data.organic || []).map((item, index) => ({
      ...
      position: Number(item.position) || ((Math.max(1, page) - 1) * resultCount) + index + 1,
    }));
  } finally {
    clearTimeout(timer);
  }
...
async function validateSourcePageHttp(url: string): Promise<ValidatedSourcePage> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
    const response = await fetch(url, {
...
let playwrightUnavailableWarningShown = false;

function looksLikeJavascriptShell(page: ValidatedSourcePage): boolean {
  if (!page.rawHtml) return true;
  const html = page.rawHtml;
  const visibleText = page.text.replace(/\s+/g, ' ').trim();
  const hasProductData = /"@type"\s*:\s*"Product"|itemtype=["'][^"']*Product|\b(?:model|mpn|sku|part[ -]?number)\b/i.test(html);
  return !hasProductData && (visibleText.length < 350 || /enable javascript|javascript is required|__next_data__|id=["'](?:root|app)["']/i.test(html));
}

async function renderSourcePage(url: string): Promise<ValidatedSourcePage | null> {
  if (!PLAYWRIGHT_FALLBACK) return null;
  try {
    if (!playwrightBrowser) {
      // Dùng tên module động để V7 vẫn type-check/chạy được khi chưa cài Playwright.
    ...
    await page.route('**/*', (route: any) => {
      ...
      if (['font', 'media'].includes(resourceType)) route.abort();
      else route.continue();
    });
    ...
    await context.close();
    if (status >= 400 || !rawHtml) return null;
    return {
      ok: true,
      contentType,
      finalUrl,
      text: htmlToEvidenceText(rawHtml),
      contentHash: crypto.createHash('sha256').update(rawHtml).digest('hex'),
      rawHtml,
      rendered: true,
    };
  } catch (error) {
...
async function validateSourcePage(url: string): Promise<ValidatedSourcePage> {
  const direct = await validateSourcePageHttp(url);
  if (!PLAYWRIGHT_FALLBACK || (direct.ok && !looksLikeJavascriptShell(direct))) return direct;
  const rendered = await renderSourcePage(url);
...
// =============================================================================
// V7 Free Mode — khám phá catalog thật từ trang hãng
// =============================================================================
let discoveryCache: DiscoveryCache = {};
let discoveryScanCache: DiscoveryScanCache = {};
let discoveryQueryCount = 0;
const discoveryDiagnostics: Record<string, DiscoveryDiagnostics> = {};

function categoryDiagnostics(slug: string): DiscoveryDiagnostics {
  if (!discoveryDiagnostics[slug]) {
    discoveryDiagnostics[slug] = {
      searchRequests: 0,
      searchResults: 0,
      sitemapUrls: 0,
      categoryUrls: 0,
  ...
  return discoveryDiagnostics[slug];
}

function rejectDiscovery(slug: string, reason: string): DiscoveryPageOutcome {
  const diagnostics = categoryDiagnostics(slug);
  diagnostics.rejected[reason] = (diagnostics.rejected[reason] || 0) + 1;
  return { product: null, reason };
}
...
function assertTargetConfig(): void {
  ...
  if (CATEGORY_TARGETS.length !== 54) {
    throw new Error(`Cấu hình V8 phải có 54 danh mục lá, hiện có ${CATEGORY_TARGETS.length}.`);
  }
  ...
  for (const item of CATEGORY_TARGETS) {
    if (slugSet.has(item.slug)) throw new Error(`Trùng category slug V8: ${item.slug}`);
...
function estimatedDiscoveryRequests(): number {
  return CATEGORY_TARGETS.reduce((total, item) => total + item.brands.reduce(
    (brandTotal, brand) => brandTotal + (BRAND_DOMAINS[brand] || []).length * item.searchTerms.length * DISCOVERY_PAGES_PER_QUERY,
    0,
  ), 0);
}

async function loadDiscoveryCache(): Promise<void> {
  const [catalogRaw, scanRaw] = await Promise.all([
    fs.readFile(DISCOVERY_CACHE_FILE, 'utf8').catch(() => ''),
    fs.readFile(DISCOVERY_SCAN_CACHE_FILE, 'utf8').catch(() => ''),
  ]);
  try {
    const parsed = JSON.parse(catalogRaw);
    discoveryCache = parsed && typeof parsed === 'object' ? parsed : {};
  } catch { discoveryCache = {}; }
  try {
    const parsed = JSON.parse(scanRaw);
    discoveryScanCache = parsed && typeof parsed === 'object' ? parsed : {};
  } catch { discoveryScanCache = {}; }
}

async function saveDiscoveryCache(): Promise<void> {
  const catalogTemp = `${DISCOVERY_CACHE_FILE}.tmp`;
  const scanTemp = `${DISCOVERY_SCAN_CACHE_FILE}.tmp`;
  await Promise.all([
    fs.writeFile(catalogTemp, JSON.stringify(discoveryCache, null, 2), 'utf8'),
    fs.writeFile(scanTemp, JSON.stringify(discoveryScanCache, null, 2), 'utf8'),
  ]);
  await Promise.all([
    fs.rename(catalogTemp, DISCOVERY_CACHE_FILE),
    fs.rename(scanTemp, DISCOVERY_SCAN_CACHE_FILE),
  ]);
}

function flattenJsonLd(value: unknown, seen = new Set<object>()): Record<string, any>[] {
  if (Array.isArray(value)) return value.flatMap((item) => flattenJsonLd(item, seen));
  if (!value || typeof value !== 'object' || seen.has(value as object)) return [];
  seen.add(value as object);
  const object = value as Record<string, any>;
  return [object, ...Object.values(object).flatMap((item) => flattenJsonLd(item, seen))];
}
...
function assertDiscoveryParser(): void {
  ...
  const nested = `<script type="application/ld+json">{"@type":"WebPage","mainEntity":{"@type":"https://schema.org/Product","name":"Cisco C9200L-24P-4G","model":"C9200L-24P-4G"}}</script>`;
  if (!parseJsonLd(nested).some((node) => schemaTypeIncludes(node, 'Product'))) {
    throw new Error('Discovery parser không đọc được Product lồng trong mainEntity.');
  }
  const labeled = `<h1>Cisco Catalyst 9200L</h1><p>Part Number: C9200L-24P-4G</p><img itemprop="image" src="/C9200L-24P-4G.png" alt="Cisco C9200L-24P-4G">`;
  if (labeledPartNumbers(labeled, htmlToEvidenceText(labeled), 'Cisco')[0] !== 'C9200L-24P-4G') {
    throw new Error('Discovery parser không đọc được part number hiển thị trên trang hãng.');
  }
}
...
function schemaTypeIncludes(node: Record<string, any>, expected: string): boolean {
  ...
  return types.some((value) => {
    ...
    return normalized === normalizedExpected || normalized.endsWith(`/${normalizedExpected}`);
  });
}
...
function productNodePartNumber(node: Record<string, any>, brand: string, pageText: string): string {
  ...
  const additional = (Array.isArray(node.additionalProperty) ? node.additionalProperty : [node.additionalProperty])
    .filter((item) => item && typeof item === 'object' && /^(?:mpn|model|sku|part(?: number)?|product code)$/i.test(String(item.name || item.propertyID || '').trim()))
    .map((item) => item.value || item.valueReference?.name);
  const candidates = [node.mpn, modelValue, node.productID, node.sku, ...additional]
...
function labeledPartNumbers(html: string, pageText: string, brand: string): string[] {
  const values: string[] = [];
  const metaPatterns = [
    /<meta\b[^>]*(?:itemprop|property|name)=["'](?:mpn|sku|model|product:id|product:retailer_item_id)["'][^>]*content=["']([^"']+)["'][^>]*>/gi,
    /<meta\b[^>]*content=["']([^"']+)["'][^>]*(?:itemprop|property|name)=["'](?:mpn|sku|model|product:id|product:retailer_item_id)["'][^>]*>/gi,
  ];
  for (const pattern of metaPatterns) {
    for (const match of html.matchAll(pattern)) values.push(match[1]);
  }
  const labelPattern = /(?:^|\n|[>;])\s*(?:model(?:\s*(?:name|number|no\.?))?|mpn|sku|part\s*(?:number|no\.?)|product\s*(?:code|number)|mã\s+sản\s+phẩm)\s*[:#-]?\s*([A-Z0-9][A-Z0-9+._\/-]*(?:\s+(?:GEN\s*\d+|G\d+|PLUS|PRO|MAX))?)/gim;
  for (const match of pageText.matchAll(labelPattern)) values.push(match[1]);
  return [...new Set(values
    .map(cleanPartNumberCandidate)
    .filter((value) => looksLikeRealPartNumber(value, brand, pageText)))];
}

function extractPageImage(html: string, pageUrl: string, productName: string): string {
  const direct = metaContent(html, 'og:image')
    || metaContent(html, 'twitter:image')
    || metaContent(html, 'twitter:image:src');
  if (direct && !/logo|icon|sprite|avatar|placeholder|default[-_]?image/i.test(direct)) {
    return absoluteFromPage(direct, pageUrl);
  }
  const candidates: Array<{ url: string; score: number }> = [];
  const regex = /<img\b([^>]*?)>/gi;
  for (const match of html.matchAll(regex)) {
    const attributes = match[1];
    const src = attributes.match(/(?:src|data-src|data-original)=["']([^"']+)["']/i)?.[1];
    if (!src) continue;
    const alt = attributes.match(/alt=["']([^"']*)["']/i)?.[1] || '';
    const url = absoluteFromPage(src, pageUrl);
    if (!url || /logo|icon|sprite|avatar|placeholder|loading/i.test(`${url} ${alt}`)) continue;
    const matchEvidence = evaluateTextMatch(`${alt} ${url}`, productName, detectBrand(productName));
    const itemProp = /itemprop=["']image["']/i.test(attributes);
    candidates.push({ url, score: matchEvidence.score + (itemProp ? 30 : 0) });
  }
...
function discoveryScanKey(categorySlug: string, brand: string, url: string): string {
  const normalizedUrl = url.replace(/[?#].*$/, '').replace(/\/$/, '').toLowerCase();
  return `${categorySlug}::${slugify(brand)}::${crypto.createHash('sha1').update(normalizedUrl).digest('hex')}`;
}

function scanCacheIsFresh(row: DiscoveryScanCache[string] | undefined): boolean {
  if (!row || row.parserVersion !== DISCOVERY_PARSER_VERSION || REVALIDATE_DISCOVERY_CACHE) return false;
  const ageMs = Date.now() - new Date(row.checkedAt).getTime();
  return Number.isFinite(ageMs) && ageMs <= DISCOVERY_CACHE_MAX_AGE_DAYS * 86_400_000;
}
...
const sitemapCatalogCache = new Map<string, Promise<string[]>>();

async function fetchDiscoveryText(url: string, maxBytes = 5 * 1024 * 1024): Promise<string> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
    const response = await fetch(url, {
      ...
      headers: {
        'User-Agent': CRAWLER_USER_AGENT,
        'Accept': 'application/xml,text/xml,text/plain,text/html;q=0.5,*/*;q=0.1',
      },
      signal: controller.signal,
    });
    if (!response.ok) {
    ...
    const bytes = await readResponseBytes(response, maxBytes, true);
...
function sitemapUrlScore(url: string): number {
  const normalized = url.toLowerCase();
  let score = 0;
  if (/product|products|catalog|shop|item|detail|sku|model/.test(normalized)) score += 20;
  if (/news|blog|press|event|career|legal|privacy|login|account|cart/.test(normalized)) score -= 30;
  if (/\.pdf(?:$|[?#])/.test(normalized)) score -= 15;
  return score;
}
...
async function crawlOfficialSitemaps(domain: string): Promise<string[]> {
  const existing = sitemapCatalogCache.get(domain);
  if (existing) return existing;
  const pending = (async () => {
    const robotsUrl = `https://${domain}/robots.txt`;
    const robots = await fetchDiscoveryText(robotsUrl, 512 * 1024);
    const declared = robots.split(/\r?\n/)
      .map((line) => line.match(/^\s*sitemap\s*:\s*(https?:\/\/\S+)/i)?.[1])
      .filter((value): value is string => Boolean(value));
    const queue = [...new Set([...declared, `https://${domain}/sitemap.xml`, `https://${domain}/sitemap_index.xml`])];
    const visited = new Set<string>();
    const pageUrls = new Set<string>();

    while (queue.length > 0 && visited.size < MAX_SITEMAP_FILES_PER_DOMAIN && pageUrls.size < MAX_SITEMAP_URLS_PER_DOMAIN) {
      const sitemapUrl = queue.shift()!;
      if (visited.has(sitemapUrl) || /\.gz(?:$|[?#])/i.test(sitemapUrl)) continue;
      visited.add(sitemapUrl);
      const xml = await fetchDiscoveryText(sitemapUrl);
      if (!xml) continue;
      const locations = xmlLocations(xml)
        .filter((url) => hostMatches(hostname(url), domain));
      const isIndex = /<sitemapindex\b/i.test(xml);
      if (isIndex) {
        const childSitemaps = locations
          .filter((url) => /\.xml(?:$|[?#])|sitemap/i.test(url))
          .sort((a, b) => sitemapUrlScore(b) - sitemapUrlScore(a));
        queue.push(...childSitemaps);
        continue;
      }
      for (const url of locations.sort((a, b) => sitemapUrlScore(b) - sitemapUrlScore(a))) {
        if (pageUrls.size >= MAX_SITEMAP_URLS_PER_DOMAIN) break;
        pageUrls.add(url);
      }
    }
    return [...pageUrls].sort((a, b) => sitemapUrlScore(b) - sitemapUrlScore(a));
  })();
  sitemapCatalogCache.set(domain, pending);
  return pending;
}

function targetUrlScore(url: string, targetItem: CategoryTarget): number {
  const text = normalizeEvidenceText(safeDecodeUrl(url)).toLowerCase();
  const stop = new Set(['product', 'products', 'official', 'business', 'enterprise', 'device', 'equipment']);
  const tokens = normalizeEvidenceText(`${targetItem.category} ${targetItem.searchTerms.join(' ')}`)
    .toLowerCase()
    .split(' ')
    .filter((token) => token.length >= 3 && !stop.has(token));
  return sitemapUrlScore(url) + tokens.reduce((score, token) => score + (text.includes(token) ? 5 : 0), 0);
}

async function sitemapCandidates(targetItem: CategoryTarget): Promise<Array<{ brand: string; result: WebSearchResult }>> {
  if (!DISCOVERY_USE_SITEMAPS) return [];
  const jobs = targetItem.brands.flatMap((brand) =>
    (BRAND_DOMAINS[brand] || []).map((domain) => ({ brand, domain })),
  );
  const batches = await mapConcurrent(jobs, DISCOVERY_CONCURRENCY, async ({ brand, domain }) => {
    try {
      const rows: Array<{ brand: string; result: WebSearchResult }> = [];
      const urls = await crawlOfficialSitemaps(domain);
      const perDomainLimit = Math.max(10, Math.ceil(targetItem.quota / Math.max(1, targetItem.brands.length)) * 5);
      for (const [index, { item: url }] of urls
        .map((item) => ({ item, score: targetUrlScore(item, targetItem) }))
        ...
        rows.push({
          brand,
          result: {
            title: safeDecodeUrl(new URL(url).pathname.split('/').filter(Boolean).at(-1) || brand).replace(/[-_]+/g, ' '),
            link: url,
            snippet: `URL sản phẩm tiềm năng từ sitemap chính hãng ${domain}.`,
            position: index + 1,
          },
        });
      }
      ...
      return [] as Array<{ brand: string; result: WebSearchResult }>;
    }
  });
  return batches.flat();
}

type FreeCatalogCandidate = {
  title: string;
  url: string;
  context: string;
};

const officialCatalogPageCache = new Map<string, Promise<FreeCatalogCandidate[]>>();
const publicCatalogApiCache = new Map<string, Promise<FreeCatalogCandidate[]>>();

function officialHttpUrl(rawUrl: string, baseUrl: string, domain: string): string | null {
  try {
    if (!rawUrl || /^(?:#|mailto:|tel:|javascript:|data:)/i.test(rawUrl)) return null;
    const parsed = new URL(decodeHtmlEntities(rawUrl), baseUrl);
    if (!/^https?:$/.test(parsed.protocol) || !hostMatches(parsed.hostname, domain)) return null;
    parsed.hash = '';
    if (/\.(?:css|js|json|xml|jpe?g|png|webp|avif|svg|ico|woff2?|ttf|mp4|zip)(?:$|\?)/i.test(parsed.href)) return null;
    if (/\/(?:login|account|cart|checkout|privacy|legal|career|news|blog|press|event)(?:\/|$)/i.test(parsed.pathname)) return null;
    return parsed.href;
  } catch {
    return null;
  }
...
function htmlCatalogLinks(html: string, baseUrl: string, domain: string): FreeCatalogCandidate[] {
  const rows = new Map<string, FreeCatalogCandidate>();
  for (const match of html.matchAll(/<a\b[^>]*href\s*=\s*["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi)) {
    const url = officialHttpUrl(match[1], baseUrl, domain);
    if (!url) continue;
    const title = htmlToEvidenceText(match[2]).replace(/\s+/g, ' ').trim();
    const key = url.replace(/[?&](?:page|p)=\d+/gi, '').replace(/\/$/, '').toLowerCase();
    if (!rows.has(key)) rows.set(key, { title, url, context: `${title} ${url}` });
  }
  return [...rows.values()];
}

function genericCatalogLinkScore(candidate: FreeCatalogCandidate): number {
  const text = normalizeEvidenceText(candidate.context).toLowerCase();
  let score = sitemapUrlScore(candidate.url);
  if (/product|products|catalog|shop|solution|hardware|device|equipment|series|model/.test(text)) score += 12;
  if (/support|download|documentation/.test(text)) score += 3;
  if (/login|account|cart|checkout|privacy|legal|career|news|blog|press|event/.test(text)) score -= 50;
  return score;
}

async function crawlOfficialCatalogPages(domain: string): Promise<FreeCatalogCandidate[]> {
  const existing = officialCatalogPageCache.get(domain);
  if (existing) return existing;
  const pending = (async () => {
    const rootPaths = ['/', '/products', '/product', '/en/products', '/business/products', '/catalog', '/shop'];
    const queue = rootPaths.map((pathname) => `https://${domain}${pathname}`);
    const visited = new Set<string>();
    const found = new Map<string, FreeCatalogCandidate>();

    while (queue.length > 0 && visited.size < MAX_CATEGORY_PAGES_PER_DOMAIN && found.size < MAX_CATEGORY_URLS_PER_DOMAIN) {
      const pageUrl = queue.shift()!;
      const visitKey = pageUrl.replace(/[?#].*$/, '').replace(/\/$/, '').toLowerCase();
      if (visited.has(visitKey)) continue;
      visited.add(visitKey);
      const html = await fetchDiscoveryText(pageUrl, MAX_SOURCE_BYTES);
      if (!html || !/<(?:html|a)\b/i.test(html)) continue;
      const links = htmlCatalogLinks(html, pageUrl, domain);
      for (const link of links) {
        const key = link.url.replace(/\/$/, '').toLowerCase();
        if (!found.has(key)) found.set(key, link);
      }
      const next = links
        .map((item) => ({ item, score: genericCatalogLinkScore(item) }))
        .filter((item) => item.score > 0)
        .sort((a, b) => b.score - a.score)
        .slice(0, 80)
        .map(({ item }) => item.url);
  ...
  officialCatalogPageCache.set(domain, pending);
  return pending;
}

function catalogApiRows(payload: any, domain: string): FreeCatalogCandidate[] {
  ...
  for (const item of source) {
    ...
    const title = String(item.name || item.title?.rendered || item.title || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
    const handle = String(item.handle || item.slug || '').trim();
    const rawUrl = String(item.permalink || item.url || item.link || (handle ? `https://${domain}/products/${handle}` : ''));
    const url = officialHttpUrl(rawUrl, `https://${domain}/`, domain);
    if (!title || !url) continue;
    const contextFields = [title, item.type, item.product_type, item.vendor, item.sku, item.model, item.mpn, item.tags];
    if (Array.isArray(item.categories)) contextFields.push(item.categories.map((row: any) => row?.name || row).join(' '));
    const context = contextFields.flat().filter(Boolean).map(String).join(' ');
    rows.set(url.replace(/\/$/, '').toLowerCase(), { title, url, context });
  }
  return [...rows.values()];
}

async function fetchPublicCatalogJson(url: string): Promise<any | null> {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), FETCH_TIMEOUT_MS);
  try {
    const response = await fetch(url, {
    ...
    const bytes = await readResponseBytes(response, 12 * 1024 * 1024, true);
...
async function crawlPublicCatalogApis(domain: string): Promise<FreeCatalogCandidate[]> {
  const existing = publicCatalogApiCache.get(domain);
  if (existing) return existing;
  const pending = (async () => {
    const endpoints = [`https://${domain}/products.json?limit=250`];
    for (let page = 1; page <= PUBLIC_API_MAX_PAGES; page += 1) {
      endpoints.push(`https://${domain}/wp-json/wc/store/v1/products?per_page=100&page=${page}`);
      endpoints.push(`https://${domain}/wp-json/wp/v2/product?per_page=100&page=${page}`);
    }
    const payloads = await mapConcurrent(endpoints, Math.min(3, DISCOVERY_CONCURRENCY), fetchPublicCatalogJson);
    const rows = new Map<string, FreeCatalogCandidate>();
    for (const payload of payloads) {
      for (const row of catalogApiRows(payload, domain)) rows.set(row.url.replace(/\/$/, '').toLowerCase(), row);
    }
    return [...rows.values()];
  })();
  publicCatalogApiCache.set(domain, pending);
  return pending;
}

function targetCandidateScore(candidate: FreeCatalogCandidate, targetItem: CategoryTarget): number {
  return targetUrlScore(`${candidate.url} ${candidate.title} ${candidate.context}`, targetItem);
}

async function categoryPageCandidates(targetItem: CategoryTarget): Promise<Array<{ brand: string; result: WebSearchResult }>> {
  if (!DISCOVERY_USE_CATEGORY_PAGES) return [];
  const jobs = targetItem.brands.flatMap((brand) => (BRAND_DOMAINS[brand] || []).map((domain) => ({ brand, domain })));
  const batches = await mapConcurrent(jobs, DISCOVERY_CONCURRENCY, async ({ brand, domain }) => {
    const rows = await crawlOfficialCatalogPages(domain);
    const limit = Math.max(15, Math.ceil(targetItem.quota / Math.max(1, targetItem.brands.length)) * 7);
    return rows
      .map((item) => ({ item, score: targetCandidateScore(item, targetItem) }))
      .filter((item) => item.score > 0)
      .sort((a, b) => b.score - a.score)
      .slice(0, limit)
      .map(({ item }, index) => ({
        brand,
        result: { title: item.title || safeDecodeUrl(item.url), link: item.url, snippet: `URL từ category chính hãng ${domain}.`, position: index + 20_000 },
      }));
  });
  return batches.flat();
}

async function publicApiCandidates(targetItem: CategoryTarget): Promise<Array<{ brand: string; result: WebSearchResult }>> {
  if (!DISCOVERY_USE_PUBLIC_CATALOG_API) return [];
  const jobs = targetItem.brands.flatMap((brand) => (BRAND_DOMAINS[brand] || []).map((domain) => ({ brand, domain })));
  const batches = await mapConcurrent(jobs, DISCOVERY_CONCURRENCY, async ({ brand, domain }) => {
    const rows = await crawlPublicCatalogApis(domain);
    const limit = Math.max(15, Math.ceil(targetItem.quota / Math.max(1, targetItem.brands.length)) * 7);
    return rows
      .map((item) => ({ item, score: targetCandidateScore(item, targetItem) }))
      .filter((item) => item.score > 0)
      ...
      .map(({ item }, index) => ({
        brand,
        result: { title: item.title, link: item.url, snippet: `Sản phẩm từ API catalog công khai của ${domain}.`, position: index + 30_000 },
      }));
  });
  return batches.flat();
}
...
async function discoverFromOfficialPage(
...
): Promise<DiscoveryPageOutcome> {
  const diagnostics = categoryDiagnostics(targetItem.slug);
  diagnostics.pagesFetched += 1;
  const officialDomains = BRAND_DOMAINS[brand] || [];
  if (!officialDomains.some((domain) => hostMatches(hostname(result.link), domain))) {
    return rejectDiscovery(targetItem.slug, 'result-not-official-domain');
  }
  const page = await validateSourcePage(result.link);
  if (page.rendered) diagnostics.renderedPages += 1;
  if (!page.ok || !page.rawHtml) return rejectDiscovery(targetItem.slug, 'page-fetch-or-content-type-failed');
  if (!officialDomains.some((domain) => hostMatches(hostname(page.finalUrl), domain))) {
    return rejectDiscovery(targetItem.slug, 'redirected-outside-official-domain');
  }
  ...
  if (discontinued && !INCLUDE_DISCONTINUED_PRODUCTS) return rejectDiscovery(targetItem.slug, 'discontinued-or-eol');
  const labeledCandidates = labeledPartNumbers(html, pageText, brand);
  let lastReason = 'missing-real-part-number';

  for (const node of candidateNodes) {
    let partNumber = productNodePartNumber(node, brand, pageText) || labeledCandidates[0] || '';
    ...
    if (!partNumber) {
      lastReason = 'missing-real-part-number';
      continue;
    }
    ...
    if (!match.exactModel && match.score < 92) {
      lastReason = 'model-not-confirmed-on-page';
      continue;
    }
    const imageUrl = absoluteFromPage(node.image, page.finalUrl) || extractPageImage(html, page.finalUrl, name);
    if (!imageUrl || isBlockedSource(imageUrl)) {
      lastReason = 'missing-product-image';
      continue;
    }
    ...
    if (!officialCanonical) {
      lastReason = 'canonical-outside-official-domain';
      continue;
    }
    ...
    const evidenceScore = 70
      ...
      + (labeledCandidates.includes(partNumber) ? 4 : 0)
      + (datasheetUrl ? 5 : 0)
  ...
  return rejectDiscovery(targetItem.slug, lastReason);
}
...
async function discoverCategory(targetItem: CategoryTarget, excludedIdentities: ReadonlySet<string>): Promise<DiscoveredProduct[]> {
  ...
  const diagnostics = categoryDiagnostics(targetItem.slug);
  const searchJobs = SERPER_ENABLED
    ? targetItem.brands.flatMap((brand) =>
      (BRAND_DOMAINS[brand] || []).flatMap((domain) =>
        targetItem.searchTerms.map((term) => ({ brand, term, domain })),
      ),
    )
    : [];
  const sitemapPromise = sitemapCandidates(targetItem);
  const categoryPagePromise = categoryPageCandidates(targetItem);
  const publicApiPromise = publicApiCandidates(targetItem);
  const searchResults: Array<{ brand: string; result: WebSearchResult }> = [];
  let activeSearchJobs = searchJobs;
  for (let page = 1; page <= DISCOVERY_PAGES_PER_QUERY && activeSearchJobs.length > 0; page += 1) {
    const remainingBudget = MAX_DISCOVERY_QUERIES - discoveryQueryCount;
    if (remainingBudget <= 0) break;
    const requestJobs = activeSearchJobs.slice(0, remainingBudget);
    discoveryQueryCount += requestJobs.length;
    diagnostics.searchRequests += requestJobs.length;
    const requestResults = await mapConcurrent(requestJobs, DISCOVERY_CONCURRENCY, async (job) => {
      // Không ép trang phải chứa chữ "model/sku/mpn" ngay trong kết quả Google;
      // nhiều website hãng chỉ đặt mã ở H1/URL. Parser phía dưới mới chịu trách
      // nhiệm xác minh part number trên chính trang sản phẩm.
      const query = `${job.domain} ${job.term}`;
      try {
        const localVendor = /\.vn$/i.test(job.domain);
        const results = await searchGoogleWeb(query, DISCOVERY_RESULTS_PER_QUERY, page, localVendor
          ? { gl: 'vn', hl: 'vi' }
          : { gl: DISCOVERY_GL, hl: DISCOVERY_HL });
        return { job, results, failed: false };
      } catch {
        return { job, results: [] as WebSearchResult[], failed: true };
      }
    });
    activeSearchJobs = [];
    for (const { job, results, failed } of requestResults) {
      if (failed) {
        diagnostics.rejected['search-request-failed'] = (diagnostics.rejected['search-request-failed'] || 0) + 1;
        continue;
      }
      diagnostics.searchResults += results.length;
      if (results.length >= Math.min(10, DISCOVERY_RESULTS_PER_QUERY)) activeSearchJobs.push(job);
      for (const result of results) {
        if ((BRAND_DOMAINS[job.brand] || []).some((domain) => hostMatches(hostname(result.link), domain))) {
          searchResults.push({ brand: job.brand, result });
        } else {
          diagnostics.rejected['search-result-outside-official-domain'] = (diagnostics.rejected['search-result-outside-official-domain'] || 0) + 1;
        }
      }
    }
  }

  const [fromSitemaps, fromCategoryPages, fromPublicApis] = await Promise.all([
    sitemapPromise,
    categoryPagePromise,
    publicApiPromise,
  ]);
  diagnostics.sitemapUrls += fromSitemaps.length;
  diagnostics.categoryUrls += fromCategoryPages.length;
  diagnostics.publicApiUrls += fromPublicApis.length;
  searchResults.push(...fromSitemaps.map((item) => ({
    brand: item.brand,
    result: { ...item.result, position: item.result.position + 10_000 },
  })));
  searchResults.push(...fromCategoryPages, ...fromPublicApis);
  ...
  diagnostics.uniqueCandidates = uniquePages.size;
  const candidates = [...uniquePages.values()]
    .filter((item) => {
      const cachedScan = discoveryScanCache[discoveryScanKey(targetItem.slug, item.brand, item.result.link)];
      if (scanCacheIsFresh(cachedScan) && !cachedScan.accepted) {
        diagnostics.rejected['cached-rejection-skipped'] = (diagnostics.rejected['cached-rejection-skipped'] || 0) + 1;
        return false;
      }
      return true;
    })
  ...
  for (let start = 0; start < candidates.length && selected.size < targetItem.quota; start += pageBatchSize) {
    ...
    const outcomes = await mapConcurrent(pageBatch, DISCOVERY_CONCURRENCY, async (item) => {
      try {
        return { item, outcome: await discoverFromOfficialPage(targetItem, item.brand, item.result) };
      } catch {
        return { item, outcome: rejectDiscovery(targetItem.slug, 'unexpected-parser-error') };
      }
    });
    for (const { item, outcome } of outcomes) {
      discoveryScanCache[discoveryScanKey(targetItem.slug, item.brand, item.result.link)] = {
        parserVersion: DISCOVERY_PARSER_VERSION,
        categorySlug: targetItem.slug,
        brand: item.brand,
        url: item.result.link,
        accepted: Boolean(outcome.product),
...
async function discoverProductCatalog(): Promise<CatalogGroup[]> {
  ...
  for (const targetItem of CATEGORY_TARGETS) {
    ...
    reportRows.push({
      ...
      diagnostics: categoryDiagnostics(targetItem.slug),
      products: accepted,
    });
    const diagnostics = categoryDiagnostics(targetItem.slug);
    const topRejected = Object.entries(diagnostics.rejected)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
    ...
    console.log(
      ...
      `${topRejected ? ` | loại: ${topRejected}` : ''}.`,
    );
    await fs.writeFile(DISCOVERY_REPORT_FILE, JSON.stringify({
      status: 'running',
      generatedAt: new Date().toISOString(),
      completedCategories: reportRows.length,
      totalCategories: CATEGORY_TARGETS.length,
      acceptedSoFar: groups.reduce((sum, group) => sum + group.products.length, 0),
      target: TARGET_TOTAL_PRODUCTS,
      discoveryQueriesUsed: discoveryQueryCount,
      discoveryMode: FREE_DISCOVERY_MODE ? 'free-official-sources-first' : 'custom',
      serperEnabled: SERPER_ENABLED,
  ...
  const report = {
    ...
    discoveryQueriesUsed: discoveryQueryCount,
    discoveryPagesPerQuery: DISCOVERY_PAGES_PER_QUERY,
    discoveryLocale: { gl: DISCOVERY_GL, hl: DISCOVERY_HL },
    sitemapDiscoveryEnabled: DISCOVERY_USE_SITEMAPS,
    categoryPageDiscoveryEnabled: DISCOVERY_USE_CATEGORY_PAGES,
    publicCatalogApiDiscoveryEnabled: DISCOVERY_USE_PUBLIC_CATALOG_API,
    playwrightFallbackEnabled: PLAYWRIGHT_FALLBACK,
    serperEnabled: SERPER_ENABLED,
    discoveryMode: FREE_DISCOVERY_MODE ? 'free-official-sources-first' : 'custom',
    officialOnly: true,
...
async function resolveProductEvidence(productName: string, image: VerifiedImage): Promise<ProductEvidenceBundle> {
  ...
  if (RESOLVE_OFFICIAL_SOURCES && SERPER_ENABLED) {
    ...
    const resultSets = await Promise.all(queries.map((query) => searchGoogleWeb(query, 10, 1, { gl: 'us', hl: 'en' })));
...
type CategorySeoPayload = {
  ...
  reviewedAt: string;
  menuType?: 'product-category' | 'brand';
  brandFilter?: string;
...
function buildCategorySeo(pathNames: string[]): CategorySeoPayload {
  ...
  const nextLevelNames = [...new Set(descendantGroups
    ...
    .filter(Boolean))];
  const relatedCategories: Array<{ name: string; url: string }> = nextLevelNames.map((childName) => ({
    name: childName,
    url: `${SITE_ORIGIN}/products/category/${slugify(childName)}`,
  }));
  // Khi đang ở đúng category lá, thêm hãng sản xuất làm cấp con tiếp theo:
  // Cáp mạng -> CommScope/Panduit..., Router -> MikroTik/DrayTek...
  const leafGroups = descendantGroups.filter((group) =>
    categoryPath(group.slug, group.category).join(' > ') === pathNames.join(' > '),
  );
  for (const group of leafGroups) {
    for (const brand of buildGroupBrandProductMap(group).keys()) {
      relatedCategories.push({
        name: brand,
        url: `${SITE_ORIGIN}/products/category/${group.slug}-${slugify(brand)}`,
      });
    }
  }
...
function buildGroupBrandProductMap(group: CatalogGroup): Map<string, string[]> {
...
function buildBrandCategorySeo(
  ...
  const basePath = categoryPath(group.slug, group.category);
  const pathNames = [...basePath, brand];
  const name = `${group.category} ${brand}`;
  const slug = `${group.slug}-${slugify(brand)}`;
  if (allProducts.length === 0) throw new Error(`Không có sản phẩm cho submenu ${group.category} > ${brand}`);

  const reviewedAt = new Date().toISOString();
  const canonicalPath = `/products/category/${slug}`;
  const canonicalUrl = `${SITE_ORIGIN}${canonicalPath}`;
  const focusKeyword = `${group.category} ${brand} chính hãng`;
  const h1 = `${group.category} ${brand} Chính Hãng`;
  const metaTitle = clip(`${group.category} ${brand} Chính Hãng | CTC`, 60);
  const metaDescription = clip(
    `Danh mục ${group.category} hãng ${brand} tại CTC gồm ${allProducts.length} model có mã sản phẩm riêng. Xem sản phẩm và nhận tư vấn báo giá.`,
    158,
  );

  const introParagraphs = [
    `${brand} là submenu hãng sản xuất nằm trực tiếp dưới danh mục ${group.category} trong hệ thống sản phẩm CTC. Trang hiện liên kết ${allProducts.length} model ${group.category} thuộc thương hiệu ${brand}, giúp người dùng lọc đúng nhà sản xuất sau khi đã chọn nhóm thiết bị. Cấu trúc điều hướng là ${basePath.join(' → ')} → ${brand}, đúng với cây danh mục đang hiển thị trong trang quản trị.`,
    `Submenu ${brand} được tạo từ trường brand đã chuẩn hóa của từng sản phẩm và chỉ xuất hiện khi hãng có domain chính thức trong allow-list. Script không lấy kích thước, công nghệ, series hoặc hai từ đầu bất kỳ của tên sản phẩm để tự tạo hãng. Quy tắc này giúp hạn chế submenu sai và giữ cho cây danh mục dễ quản lý khi seed lại nhiều lần.`,
    `Mỗi sản phẩm chỉ được upsert một lần theo slug. Sản phẩm vẫn giữ category chính là ${group.category}, đồng thời nhận thêm brandCategoryId trỏ tới ${group.category} → ${brand}. Vì vậy trang danh mục cha vẫn đếm đủ sản phẩm, submenu hãng đếm đúng các model ${brand}, còn URL sản phẩm không bị nhân bản hoặc phát sinh duplicate content.`,
    `Mã sản phẩm của từng model được lưu ở code, SKU, model, part number và MPN. CTC không dùng mã tuần tự tự đặt để thay cho part number của hãng. Khi so sánh các model ${brand}, khách hàng nên kiểm tra đúng mã hàng, revision, khả năng tương thích và tài liệu kỹ thuật thay vì chỉ dựa vào tên series hoặc hình thức bên ngoài.`,
    `Danh sách ${group.category} ${brand} phục vụ tra cứu catalog và yêu cầu báo giá; không mặc định xác nhận tồn kho, thời gian giao hàng, quyền phân phối hay thời hạn bảo hành. Ảnh được tìm riêng theo tên và model qua Google Images, sau đó kiểm tra URL, định dạng, kích thước, mức độ khớp model và fingerprint trước khi lưu URL ảnh gốc.`,
    `Để nhận tư vấn ${group.category} ${brand}, khách hàng nên gửi đúng part number, số lượng, hệ thống cần tích hợp, yêu cầu kỹ thuật, tiến độ và địa điểm giao nhận. Internal link của submenu đi theo luồng Category → Brand → Product → Contact, đồng thời vẫn giữ luồng News → Category → Product → Contact cho chiến lược SEO tổng thể.`
  ];
  const intro = introParagraphs.join('\n\n');
  const wordCount = intro.split(/\s+/).filter(Boolean).length;
  if (wordCount < 300 || wordCount > 600) {
    throw new Error(`Brand Category SEO ${group.category} > ${brand} phải 300–600 từ, hiện có ${wordCount} từ.`);
  }
  ...
  const relatedCategories: Array<{ name: string; url: string }> = [];
  const faq = [
    {
      question: `CTC có những sản phẩm ${group.category} ${brand} nào?`,
      answer: `Submenu hiện liên kết ${allProducts.length} model ${group.category} hãng ${brand} trong catalog tĩnh.`,
    },
    {
      question: `Mã sản phẩm ${brand} được lưu như thế nào?`,
      answer: 'Code, SKU, model, part number và MPN sử dụng mã model/part number đã khai báo, không dùng mã CTC tự sinh.',
    },
    {
      question: `Giá ${group.category} ${brand} được xác định thế nào?`,
      answer: 'Giá để ở trạng thái Liên hệ và được xác nhận theo model, số lượng, thời điểm, cấu hình cùng điều kiện giao hàng.',
    },
    {
      question: `Làm sao nhận tư vấn ${group.category} ${brand}?`,
...
<article class="category-seo-content brand-category-seo" data-category-path="${escapeHtml(pathNames.join(' > '))}">
  <header><h1>${escapeHtml(h1)}</h1></header>
  ${introParagraphs.map((paragraph) => `<p>${escapeHtml(paragraph)}</p>`).join('\n  ')}
  <section><h2>Sản phẩm ${escapeHtml(group.category)} ${escapeHtml(brand)} nổi bật</h2><ul>${productLinksHtml}</ul></section>
  ...
  const breadcrumbItems = [
    ...
    ...basePath.map((item, index) => ({
      ...
      position: index + 2,
      name: item,
      item: `${SITE_ORIGIN}/products/category/${index === basePath.length - 1 ? group.slug : slugify(item)}`,
    })),
    { '@type': 'ListItem', position: basePath.length + 2, name: brand, item: canonicalUrl },
  ];
  const structuredData = {
    '@context': 'https://schema.org',
    '@graph': [
  ...
  return {
    ...
    relatedCategories,
    suggestedNewsTopics: [
      `Cách chọn ${group.category} ${brand}`,
      `So sánh các model ${group.category} ${brand}`,
      `Checklist báo giá ${group.category} ${brand}`,
    ],
    structuredData,
    reviewedAt,
    menuType: 'brand',
    brandFilter: brand,
    productCategorySlug: group.slug,
  };
}
...
function buildAllCategorySeo(): Map<string, CategorySeoPayload> {
  ...
  for (const group of ACTIVE_PRODUCT_CATALOG) {
    for (const [brand, products] of buildGroupBrandProductMap(group)) {
      const seo = buildBrandCategorySeo(group, brand, products);
...
async function ensureCategory(
  ...
  const isBrandLeaf = categorySeo.menuType === 'brand';
  const category = await ProductCategory.findOneAndUpdate(
    { slug },
    {
      $set: {
        ...
        menuType: isBrandLeaf ? 'brand' : 'product-category',
        brandFilter: isBrandLeaf ? categorySeo.brandFilter : undefined,
        productCategorySlug: isBrandLeaf ? categorySeo.productCategorySlug : undefined,
...
async function main(): Promise<void> {
  assertTargetConfig();
  await fs.mkdir(CACHE_DIR, { recursive: true });
  ACTIVE_PRODUCT_CATALOG = buildStaticProductCatalog();
  assertCatalog();
  await fs.writeFile(DISCOVERED_CATALOG_FILE, JSON.stringify(ACTIVE_PRODUCT_CATALOG, null, 2), 'utf8');
  await fs.writeFile(DISCOVERY_REPORT_FILE, JSON.stringify({
    generatedAt: new Date().toISOString(),
    mode: 'static-catalog-no-product-crawl',
    categories: ACTIVE_PRODUCT_CATALOG.length,
    products: ACTIVE_PRODUCT_CATALOG.reduce((sum, group) => sum + group.products.length, 0),
    googleImageUrls: true,
    mirrorImages: MIRROR_IMAGES,
    catalog: ACTIVE_PRODUCT_CATALOG.map((group) => ({ slug: group.slug, category: group.category, products: group.products.length })),
  }, null, 2), 'utf8');
  ...
  const evidenceResult = STATIC_SEED_MODE
    ? new Map(flatProducts.map(({ name }) => {
      const image = imageResult.ok.get(name)!;
      return [name, {
        productName: name,
        brand: detectBrand(name),
        model: extractModel(name, detectBrand(name)),
        primarySource: unresolvedSource(name, image),
        datasheet: null,
        specifications: [],
        evidenceStatus: 'unresolved' as const,
        reviewedAt: new Date().toISOString(),
      } satisfies ProductEvidenceBundle];
    }))
    : await resolveAllEvidence(flatProducts, imageResult.ok);
  ...
  const brandCategoryIdByKey = new Map<string, mongoose.Types.ObjectId>();
  const prepared: any[] = [];
  ...
  if (!DRY_RUN) {
    ...
    if (CLEANUP_LEGACY_GLOBAL_BRAND_MENU) {
      // Dọn đúng nhánh "Thương Hiệu" độc lập do bản trước tạo. Không xóa
      // submenu hãng mới vì chúng nằm dưới category sản phẩm tương ứng.
      const obsoleteBrandCategories = await ProductCategory.deleteMany({
        $or: [
          { menuType: 'brand-root' },
          { menuType: 'brand', 'categoryPath.0': 'Thương Hiệu' },
        ],
      });
    ...
    for (const group of ACTIVE_PRODUCT_CATALOG) {
      ...
      categoryIdBySlug.set(
        group.slug,
        await ensureCategoryPath(pathNames, categorySeoByKey),
      );
      let brandMenuOrder = 1;
      for (const brand of buildGroupBrandProductMap(group).keys()) {
        const brandCategoryId = await ensureCategoryPath([...pathNames, brand], categorySeoByKey);
        brandCategoryIdByKey.set(`${group.slug}::${brand}`, brandCategoryId);
  ...
  for (const group of ACTIVE_PRODUCT_CATALOG) {
    ...
    for (const name of group.products) {
      const brand = detectBrand(name);
      const brandCategoryId = brandCategoryIdByKey.get(`${group.slug}::${brand}`);
      const partNumber = extractPartNumber(name, brand);
      ...
      prepared.push({
        ...
        brandCategoryId,
        brandCategorySlug: brandCategoryId ? `${group.slug}-${slugify(brand)}` : null,
        brandCategoryPath: brandCategoryId ? [...pathNames, brand] : [],
