 ...
 *
 * Chạy local:
 *   npx tsx server/scripts/seed-500-seo-products-real-img-geo-v9.ts
 *
 * Chạy Docker:
 *   docker compose exec app npx tsx server/scripts/seed-500-seo-products-real-img-geo-v9.ts
 */
...
const CLEAR_IMAGE_CACHE = envBool('CLEAR_IMAGE_CACHE', false);
const ALLOW_IMAGE_CACHE_WITHOUT_SERPER = envBool('ALLOW_IMAGE_CACHE_WITHOUT_SERPER', false);
...
const CLEAR_SOURCE_CACHE = envBool('CLEAR_SOURCE_CACHE', false);

const IMAGE_CONCURRENCY = envInt('IMAGE_CONCURRENCY', 3, 1, 8);
const SOURCE_CONCURRENCY = envInt('SOURCE_CONCURRENCY', 4, 1, 10);
const SERPER_RESULTS = envInt('SERPER_RESULTS', 20, 5, 20);
const SERPER_IMAGE_QUERY_LIMIT = envInt('SERPER_IMAGE_QUERY_LIMIT', 4, 1, 8);
const SERPER_MAX_RETRIES = envInt('SERPER_MAX_RETRIES', 3, 0, 6);
const SERPER_RETRY_BASE_MS = envInt('SERPER_RETRY_BASE_MS', 1500, 250, 30000);
const SERPER_MIN_INTERVAL_MS = envInt('SERPER_MIN_INTERVAL_MS', 350, 0, 5000);
...
const SEED_TAG = 'ctc-seed-500-seo-geo-real-image-v9';
const LEGACY_SEED_TAGS = [
  'ctc-seed-500-seo-geo-real-image-v4',
  'ctc-seed-500-seo-geo-real-image-v5',
  'ctc-seed-500-seo-geo-real-image-v6',
  'ctc-seed-500-seo-geo-real-image-v7',
  'ctc-seed-500-seo-geo-real-image-v8',
] as const;
const GEO_STANDARD = 'CTC-GEO-2026.5';

const CACHE_DIR = path.resolve(__dirname, '../.cache/seed-500-products-v9');
const IMAGE_CACHE_FILE = path.join(CACHE_DIR, 'image-cache-v9.json');
const SOURCE_CACHE_FILE = path.join(CACHE_DIR, 'source-cache-v9.json');
const V8_CACHE_DIR = path.resolve(__dirname, '../.cache/seed-500-products-v8');
const V8_IMAGE_CACHE_FILE = path.join(V8_CACHE_DIR, 'image-cache-v8.json');
const V8_SOURCE_CACHE_FILE = path.join(V8_CACHE_DIR, 'source-cache-v8.json');
...
function isGenericSpecToken(token: string): boolean {
  const t = token.toUpperCase().replace(/["']/g, '');
  // Các token dưới đây mô tả cấu hình/kích thước chứ không phải mã model.
  // v7 từng chọn nhầm 14INCH, 300DPI, 48PORT... làm "model chính" khiến ảnh đúng bị loại.
  if (/^\d+(?:\.\d+)?(?:W|KW|MW|V|KV|A|MA|AH|MAH|WH|KWH|VA|KVA|GB|TB|MB|MHZ|GHZ|DPI|INCH|IN|FT|KM|M|MM|CM|FO|CORE|CORES|USER|USERS|PORT|PORTS|P|U|MPPT|FXS|FXO)$/i.test(t)) return true;
  if (/^(?:CAT5E|CAT6|CAT6A|CAT7|WIFI[4567]?|GEN\d+|I[3579]|RYZEN\d*|XEON|POE\+?|FHD|4K|8K)$/i.test(t)) return true;
  if (/^\d+(?:P|PORT|PORTS|FO|CORE|CORES|USER|USERS)$/i.test(t)) return true;
  return false;
}

function modelTokenScore(token: string): number {
  const t = token.toUpperCase();
  if (isGenericSpecToken(t)) return -100;
  const hasLetter = /[A-Z]/.test(t);
  const hasDigit = /\d/.test(t);
  const hasSeparator = /[-+./]/.test(t);
  let score = 0;
  if (hasLetter && hasDigit) score += 80;
  else if (hasDigit && /^\d{4,}$/.test(t)) score += 45;
  else if (hasDigit) score += 20;
  if (hasSeparator) score += 25;
...
async function loadImageCache(): Promise<void> {
  ...
  if (CLEAR_IMAGE_CACHE) {
    imageCache = {};
    await fs.rm(IMAGE_CACHE_FILE, { force: true });
    console.log('🧹 Đã bỏ cache ảnh v9 theo CLEAR_IMAGE_CACHE=true.');
    return;
  }
  ...
  const fallbackFiles = [
    IMAGE_CACHE_FILE,
    V8_IMAGE_CACHE_FILE,
    path.resolve(__dirname, '../.cache/seed-500-products-v7/image-cache-v7.json'),
    path.resolve(__dirname, '../.cache/seed-500-products-v6/image-cache-v6.json'),
  ];
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        for (const [key, val] of Object.entries(parsed)) {
          if (!imageCache[key] && val && typeof val === 'object') {
            imageCache[key] = val as VerifiedImage;
          }
        }
      }
    } catch {
  ...
  if (Object.keys(imageCache).length > 0) {
    console.log(`♻️  Đã nạp ${Object.keys(imageCache).length} ảnh hợp lệ từ cache.`);
...
// =============================================================================
class SerperFatalError extends Error {
  constructor(message: string, public readonly status?: number) {
    super(message);
    this.name = 'SerperFatalError';
  }
}
...
let serperGate: Promise<void> = Promise.resolve();
let lastSerperRequestStartedAt = 0;

async function waitForSerperSlot(): Promise<void> {
  let release!: () => void;
  const previous = serperGate;
  serperGate = new Promise<void>((resolve) => { release = resolve; });
  await previous;
  try {
    const elapsed = Date.now() - lastSerperRequestStartedAt;
    const waitMs = Math.max(0, SERPER_MIN_INTERVAL_MS - elapsed);
    if (waitMs > 0) await sleep(waitMs);
    lastSerperRequestStartedAt = Date.now();
  } finally {
    release();
  }
}
...
function retryAfterMs(response: Response, attempt: number): number {
  ...
  return SERPER_RETRY_BASE_MS * (2 ** attempt) + Math.floor(Math.random() * 400);
}
...
async function serperPost<T>(
  ...
  if (!SERPER_API_KEY) throw new SerperFatalError('Thiếu SERPER_API_KEY.');

  let lastMessage = '';
  for (let attempt = 0; attempt <= SERPER_MAX_RETRIES; attempt += 1) {
    await waitForSerperSlot();
    serperStats.requests += 1;
    if (endpoint === 'images') serperStats.imageRequests += 1;
    else serperStats.webRequests += 1;
    ...
    try {
      ...
      lastMessage = `Serper ${endpoint} HTTP ${response.status}: ${text || response.statusText}`;

      if ([401, 402, 403].includes(response.status)) {
        serperStats.fatalErrors += 1;
      ...
      if (retryable && attempt < SERPER_MAX_RETRIES) {
        ...
        console.warn(`⚠️  ${lastMessage} — thử lại sau ${Math.ceil(waitMs / 1000)}s (${attempt + 1}/${SERPER_MAX_RETRIES}).`);
        await sleep(waitMs);
        continue;
      }
      ...
      if (retryable) {
        serperStats.fatalErrors += 1;
        throw new SerperFatalError(`${lastMessage}. Đã hết số lần retry; dừng seed để giữ cache đã cào được.`, response.status);
      }
      ...
      if (attempt < SERPER_MAX_RETRIES) {
        serperStats.retries += 1;
        const waitMs = SERPER_RETRY_BASE_MS * (2 ** attempt) + Math.floor(Math.random() * 400);
        console.warn(`⚠️  Serper ${endpoint} lỗi mạng/timeout: ${lastMessage} — thử lại sau ${Math.ceil(waitMs / 1000)}s.`);
        await sleep(waitMs);
        continue;
      }
      serperStats.fatalErrors += 1;
      throw new SerperFatalError(`Serper ${endpoint} lỗi liên tiếp: ${lastMessage}. Dừng seed để không biến lỗi API thành hàng loạt ảnh lỗi.`);
    } finally {
      clearTimeout(timer);
    }
  }

  throw new SerperFatalError(lastMessage || `Serper ${endpoint} thất bại.`);
}

async function searchGoogleImages(query: string): Promise<ImageCandidate[]> {
  const data = await serperPost<{ images?: any[] }>('images', {
    q: query,
    gl: 'vn',
    hl: 'vi',
    num: SERPER_RESULTS,
...
async function mirrorImage(url: string, productSlug: string, contentType: string): Promise<{ publicUrl: string; localPath: string; contentHash: string }> {
  ...
  try {
    ...
    return { publicUrl: `${PUBLIC_IMAGE_PREFIX}/${filename}`, localPath, contentHash: hash };
  } finally {
    clearTimeout(timer);
  }
}
...
async function resolveProductImage(productName: string): Promise<VerifiedImage> {
  ...
  for (const query of queries) {
    try {
      ...
      for (const item of ranked.slice(0, 8)) {
        ...
        if (MIRROR_IMAGES) {
          try {
            ...
            // Không lưu hotlink dễ chết trên giao diện. Thử candidate kế tiếp.
            continue;
          }
        }
      ...
      // Lỗi riêng của truy vấn/candidate: thử truy vấn kế tiếp.
    }
  }

  const detail = queryErrors.length > 0 ? ` Lỗi truy vấn: ${queryErrors.slice(-2).join(' | ')}` : '';
  throw new Error(`Không tìm được ảnh đủ khớp model cho: ${productName} sau ${queries.length} truy vấn.${detail}`);
  ...
  for (let start = 0; start < items.length; start += IMAGE_CONCURRENCY) {
    ...
    const results = await Promise.all(batch.map(async (item) => {
      try {
        ...
        if (error instanceof SerperFatalError) return { item, fatal: error } as const;
        return { item, error: error instanceof Error ? error.message : String(error) } as const;
      }
    }));

    const fatal = results.find((result): result is { item: { name: string }; fatal: SerperFatalError } => 'fatal' in result);
    if (fatal) {
      await saveImageCache();
      console.error(`\n🛑 ${fatal.fatal.message}`);
      console.error(`💾 Cache hiện đã giữ ${Object.keys(imageCache).length} ảnh; lần chạy sau KHÔNG xóa cache.`);
      console.error(`📊 Serper: requests=${serperStats.requests}, retries=${serperStats.retries}, 429=${serperStats.rateLimited}, 5xx=${serperStats.serverErrors}`);
      throw fatal.fatal;
    }

    for (const result of results) {
      if ('image' in result) ok.set(result.item.name, result.image);
      else if ('error' in result) failed.push({ name: result.item.name, error: result.error });
    }

    await saveImageCache();
    const done = Math.min(start + batch.length, items.length);
    if (done % 25 === 0 || done === items.length) {
      console.log(`🖼️  Đã kiểm tra ${done}/${items.length} sản phẩm | đạt: ${ok.size} | lỗi: ${failed.length} | Serper req: ${serperStats.requests} | retry: ${serperStats.retries} | 429: ${serperStats.rateLimited}`);
    }
  }
...
function assertNoDuplicateImages(images: Map<string, VerifiedImage>): void {
  const byIdentity = new Map<string, string[]>();
  for (const [productName, image] of images.entries()) {
    const identity = image.contentHash
      ? `hash:${image.contentHash}`
      : `url:${image.imageUrl.replace(/[?#].*$/, '').toLowerCase()}`;
    const products = byIdentity.get(identity) || [];
    products.push(productName);
    byIdentity.set(identity, products);
  }

  const duplicates = [...byIdentity.entries()].filter(([, products]) => products.length > 1);
  if (duplicates.length > 0) {
    const sample = duplicates.slice(0, 12)
      .map(([identity, products]) => `- ${identity}: ${products.join(' | ')}`)
      .join('\n');
    throw new Error(
      `Phát hiện ${duplicates.length} ảnh bị dùng lặp cho nhiều sản phẩm. V7 dừng để tránh ảnh sai model.\n${sample}`,
    );
  }
}
...
async function loadSourceCache(): Promise<void> {
  await fs.mkdir(CACHE_DIR, { recursive: true });
  if (CLEAR_SOURCE_CACHE) {
    sourceCache = {};
    await fs.rm(SOURCE_CACHE_FILE, { force: true });
    console.log('🧹 Đã bỏ cache nguồn v9 theo CLEAR_SOURCE_CACHE=true.');
    return;
  }

  const fallbackFiles = [
    SOURCE_CACHE_FILE,
    V8_SOURCE_CACHE_FILE,
    path.resolve(__dirname, '../.cache/seed-500-products-v7/source-cache-v7.json'),
    path.resolve(__dirname, '../.cache/seed-500-products-v6/source-cache-v6.json'),
  ];
  ...
  for (const file of fallbackFiles) {
    try {
      ...
      if (parsed && typeof parsed === 'object') {
        for (const [key, val] of Object.entries(parsed)) {
          if (!sourceCache[key] && val && typeof val === 'object') {
            sourceCache[key] = val as SourceEvidence;
          }
        }
      }
    } catch {
  ...
  if (Object.keys(sourceCache).length > 0) {
    console.log(`♻️  Đã nạp ${Object.keys(sourceCache).length} nguồn hợp lệ từ cache.`);
...
async function searchGoogleWeb(query: string): Promise<WebSearchResult[]> {
  ...
  const data = await serperPost<{ organic?: any[] }>('search', {
    q: query,
    gl: 'vn',
    hl: 'vi',
    num: 10,
...
async function resolveProductSource(productName: string, image: VerifiedImage): Promise<SourceEvidence> {
  ...
  // Trang chứa ảnh trên domain hãng chỉ xác nhận nguồn hình ảnh/model ở mức sơ bộ.
  // Không được tự động coi đây là nguồn thông số kỹ thuật.
  let officialImageEvidence: SourceEvidence | null = null;
  if (imageLooksLikeProductPage) {
    ...
    if (checked.ok) {
      officialImageEvidence = {
        ...
        snippet: 'Trang trên domain hãng được dùng để xác nhận hình ảnh/model; chưa đủ điều kiện xác nhận thông số kỹ thuật.',
        official: true,
  ...
  const fallback: SourceEvidence = officialImageEvidence || {
    productName,
    brand,
    model,
...
function buildProductContent(params: {
  ...
  const verifiedOfficial = Boolean(
    source.official
    && source.supportsProductFacts
    && source.httpValidated
    && source.sourceType === 'manufacturer-product-page'
  );
  const focusKeyword = verifiedOfficial ? `${name} chính hãng` : name;
  const reviewedAt = new Date().toISOString();
  const reviewedDate = reviewedAt.slice(0, 10);
  const contentVariant = stableNumber(name) % 4;

  const metaTitle = clip(
    verifiedOfficial
      ? `${name} chính hãng | Báo giá CTC`
      : `${name} | Thông tin & báo giá CTC`,
    60,
  );
  const metaDescription = clip(
    verifiedOfficial
      ? `${name}, model ${model}, thương hiệu ${brand}. Xem nguồn hãng, ứng dụng, tiêu chí lựa chọn và yêu cầu báo giá tại CTC. Giá liên hệ.`
      : `${name}, model ${model}, thương hiệu ${brand}. CTC đang bổ sung nguồn hãng để xác nhận thông số; xem ứng dụng và yêu cầu báo giá.`,
    158,
  );

  const directAnswerVariants = [
    `${name} thuộc nhóm ${categoryName}: ${profile.summary}. CTC ghi nhận model ${model}, thương hiệu ${brand}; cấu hình triển khai cần đối chiếu tài liệu của đúng model trước khi báo giá.`,
    `Với model ${model}, ${name} được xếp trong nhóm ${categoryName} và thường được xem xét cho ${profile.applications.slice(0, 3).join(', ')}. Thông số cuối cùng phải đối chiếu nguồn của ${brand}.`,
    `${name} là sản phẩm ${brand} thuộc danh mục ${categoryName}. Khi lựa chọn, cần ưu tiên ${profile.selection}; CTC chỉ chốt cấu hình sau khi kiểm tra đúng model ${model}.`,
    `CTC đang niêm yết ${name} theo model ${model} trong nhóm ${categoryName}. Nhóm sản phẩm này phục vụ ${profile.applications.slice(0, 3).join(', ')}; giá và cấu hình được xác nhận theo từng yêu cầu.`,
  ];
  const directAnswer = directAnswerVariants[contentVariant];

  const limitation = verifiedOfficial
    ? `CTC đã liên kết nguồn sản phẩm chính thức của ${brand}. Khi báo giá vẫn cần kiểm tra phiên bản, khu vực phân phối, firmware/cấu hình và tài liệu kỹ thuật đang có hiệu lực.`
    : `Seed chưa xác minh được trang sản phẩm/datasheet chính thức của đúng model ở mức đủ tin cậy. Trang vẫn có thể hiển thị trong catalog CTC nhưng được đặt noindex cho đến khi bổ sung nguồn hãng; không suy diễn cổng kết nối, công suất, chứng nhận, tồn kho hoặc thời hạn bảo hành.`;
  ...
  const sourceHtml = verifiedOfficial
...
<article class="product-seo-content product-geo-content space-y-6" data-geo-standard="${GEO_STANDARD}" data-entity-id="${canonicalUrl}#product">
  ...
  const productNode: Record<string, unknown> = {
    ...
    additionalProperty: [
      ...
      { '@type': 'PropertyValue', name: 'Mức độ xác thực thông số', value: verifiedOfficial ? 'Đã xác minh nguồn sản phẩm hãng' : 'Cần bổ sung nguồn sản phẩm hãng' },
    ],
    ...(verifiedOfficial ? { sameAs: [source.url] } : {}),
  };
  ...
  const webPageNode: Record<string, unknown> = {
    ...
    ...(verifiedOfficial ? { citation: [source.url] } : {}),
  };
  ...
  const geo = {
    ...
    sourceEvidence: {
      officialProductSource: verifiedOfficial ? {
        url: source.url,
    ...
    citationReady: verifiedOfficial,
  };
  ...
  return {
    ...
    seo: {
      ...
      secondaryKeywords: [
        `${name} giá liên hệ`,
        `${name} datasheet`,
        verifiedOfficial ? `${model} chính hãng` : `${model} thông tin sản phẩm`,
        `${categoryName} ${brand}`,
        `mua ${name} tại Việt Nam`,
      ],
      canonicalPath,
      canonicalUrl,
      robotsIndex: verifiedOfficial,
      robotsFollow: true,
      robotsReason: verifiedOfficial
        ? 'Đã xác minh nguồn sản phẩm hãng; đủ điều kiện index.'
        : 'Chưa có nguồn sản phẩm hãng đủ điều kiện; tạm noindex để tránh thin/unsupported content.',
      ogTitle: metaTitle,
...
async function main(): Promise<void> {
  ...
  console.log(`REVALIDATE_CACHE  : ${REVALIDATE_CACHE}`);
  console.log(`CLEAR_IMAGE_CACHE : ${CLEAR_IMAGE_CACHE}`);
  console.log(`IMAGE_QUERY_LIMIT : ${SERPER_IMAGE_QUERY_LIMIT}`);
  console.log(`SERPER_RETRIES    : ${SERPER_MAX_RETRIES}`);
  console.log(`SERPER_INTERVAL   : ${SERPER_MIN_INTERVAL_MS}ms`);
  console.log(`DRY_RUN           : ${DRY_RUN}`);
  console.log(`VALIDATE_ONLY     : ${VALIDATE_ONLY}`);
  console.log('════════════════════════════════════════════════════════════\n');

  if (REVALIDATE_CACHE && !SERPER_API_KEY) {
    throw new Error('REVALIDATE_CACHE=true bắt buộc phải có SERPER_API_KEY. Không thể tìm ảnh mới khi Serper API chưa cấu hình.');
  }
  if (!SERPER_API_KEY && !ALLOW_IMAGE_CACHE_WITHOUT_SERPER) {
    throw new Error('Thiếu SERPER_API_KEY. V9 mặc định không cho seed từ cache cũ. Chỉ đặt ALLOW_IMAGE_CACHE_WITHOUT_SERPER=true khi cache đã được kiểm chứng riêng.');
  }
  if (!SERPER_API_KEY && Object.keys(imageCache).length < flatProducts.length) {
    throw new Error('Cache ảnh không đủ 500 sản phẩm và không có SERPER_API_KEY để tìm lại.');
  }
  ...
  if (imageResult.failed.length > 0 || imageResult.ok.size !== 500) {
    console.error('\n❌ Một số sản phẩm chưa tìm được ảnh đúng model:');
    for (const item of imageResult.failed.slice(0, 30)) console.error(` - ${item.name}: ${item.error}`);
    if (imageResult.failed.length > 30) console.error(` ... và ${imageResult.failed.length - 30} sản phẩm khác. Xem ${IMAGE_REPORT_FILE}`);
    throw new Error(`Chưa đủ 500 ảnh hợp lệ. Đạt ${imageResult.ok.size}/500. Xem báo cáo: ${IMAGE_REPORT_FILE}`);
  }
  assertNoDuplicateImages(imageResult.ok);
  console.log('✅ Kiểm tra ảnh trùng: 500 sản phẩm không dùng lặp cùng một ảnh.');
  ...
  if (!DRY_RUN) {
    ...
    if (RESET_ALL_PRODUCTS) {
      ...
      console.log(`🗑️  Đã xóa ${result.deletedCount} sản phẩm thuộc seed v4-v9.`);
    }
  ...
  for (const group of PRODUCT_CATALOG) {
    ...
    for (const name of group.products) {
      ...
      prepared.push({
        ...
        sourceUrl: source.official ? source.url : image.sourcePage,
        sourceDomain: source.official ? source.domain : image.sourceDomain,
        ...
        dataQuality: {
          ...
          officialProductSourceResolved: Boolean(source.official && source.supportsProductFacts && source.sourceType === 'manufacturer-product-page'),
          sourceHttpValidated: source.httpValidated,
          contentDoesNotInferSpecifications: true,
          indexEligible: Boolean(source.official && source.supportsProductFacts && source.sourceType === 'manufacturer-product-page'),
  ...
  console.log('\n✅ Hoàn thành: đã upsert đúng 500 sản phẩm, giá Liên hệ, SEO + GEO 2026/AEO, nguồn và ảnh đã kiểm chứng; không chấp nhận ảnh dùng lặp.');
}
