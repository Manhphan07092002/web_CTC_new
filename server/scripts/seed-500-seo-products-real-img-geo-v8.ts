 ...
 *
 * Chạy local:
 *   npx tsx server/scripts/seed-500-seo-products-real-img-geo-v8.ts
 *
 * Chạy Docker:
 *   docker compose exec app npx tsx server/scripts/seed-500-seo-products-real-img-geo-v8.ts
 */
...
const SEED_TAG = 'ctc-seed-500-seo-geo-real-image-v8';
...
const GEO_STANDARD = 'CTC-GEO-2026.4';

const CACHE_DIR = path.resolve(__dirname, '../.cache/seed-500-products-v8');
const IMAGE_CACHE_FILE = path.join(CACHE_DIR, 'image-cache-v8.json');
const SOURCE_CACHE_FILE = path.join(CACHE_DIR, 'source-cache-v8.json');
...
async function loadImageCache(): Promise<void> {
  ...
  if (CLEAR_IMAGE_CACHE) {
    ...
    console.log('🧹 Đã bỏ cache ảnh v8 theo CLEAR_IMAGE_CACHE=true.');
    return;
  }
...
async function loadSourceCache(): Promise<void> {
  ...
  if (CLEAR_SOURCE_CACHE) {
    ...
    console.log('🧹 Đã bỏ cache nguồn v8 theo CLEAR_SOURCE_CACHE=true.');
    return;
  }
...
async function main(): Promise<void> {
  ...
  if (!SERPER_API_KEY && !ALLOW_IMAGE_CACHE_WITHOUT_SERPER) {
    throw new Error('Thiếu SERPER_API_KEY. V7 mặc định không cho seed từ cache cũ. Chỉ đặt ALLOW_IMAGE_CACHE_WITHOUT_SERPER=true khi cache đã được kiểm chứng riêng.');
  }
  ...
  if (!DRY_RUN) {
    ...
    if (RESET_ALL_PRODUCTS) {
      ...
      console.log(`🗑️  Đã xóa ${result.deletedCount} sản phẩm thuộc seed v4/v5/v6/v7.`);
    }
