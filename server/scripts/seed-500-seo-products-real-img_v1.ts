/**
 * Seed đúng 500 sản phẩm CTC chuẩn SEO + GEO/AEO và ảnh sản phẩm thực tế.
 *
 * Điểm chính:
 ...
 * - GEO cho AI gồm answer-first, entity, câu hỏi được trả lời và nguồn ảnh.
 *
...
const SEED_TAG = 'ctc-seed-500-seo-real-image-v3';

const CACHE_DIR = path.resolve(__dirname, '../.cache/seed-500-products-v3');
const IMAGE_CACHE_FILE = path.join(CACHE_DIR, 'image-cache-v3.json');
...
const GEO_CLUSTERS = [
  { name: 'Miền Bắc', provinces: ['Hà Nội', 'Hải Phòng', 'Quảng Ninh', 'Bắc Ninh', 'Hưng Yên', 'Thái Nguyên'] },
  { name: 'Bắc Trung Bộ', provinces: ['Thanh Hóa', 'Nghệ An', 'Hà Tĩnh', 'Quảng Trị', 'Huế'] },
  { name: 'Miền Trung và Tây Nguyên', provinces: ['Đà Nẵng', 'Quảng Ngãi', 'Gia Lai', 'Khánh Hòa', 'Đắk Lắk', 'Lâm Đồng'] },
  { name: 'Đông Nam Bộ', provinces: ['Thành phố Hồ Chí Minh', 'Đồng Nai', 'Tây Ninh'] },
  { name: 'Đồng bằng sông Cửu Long', provinces: ['Cần Thơ', 'An Giang', 'Đồng Tháp', 'Vĩnh Long', 'Cà Mau'] },
] as const;
...
const CATEGORY_PATHS: Record<string, string[]> = {
  ...
  'sfp-module-quang': ['HẠ TẦNG VIỄN THÔNG & CNTT', 'Thiết bị truyền dẫn quang', 'SFP'],
  'odf-tu-phan-phoi-quang': ['HẠ TẦNG VIỄN THÔNG & CNTT', 'Thiết bị truyền dẫn quang', 'ODF'],
  'voip-gateway': ['HẠ TẦNG VIỄN THÔNG & CNTT', 'Tổng đài và VoIP', 'VoIP Gateway'],
  'ip-pbx-tong-dai': ['HẠ TẦNG VIỄN THÔNG & CNTT', 'Tổng đài và VoIP', 'IP PBX'],
...
function candidateMatchesProduct(candidate: ImageCandidate, productName: string, brand: string): boolean {
  ...
  if (tokens.length === 0) {
    const words = productName.toUpperCase().split(/[^A-Z0-9]+/).filter((x) => x.length >= 4).slice(0, 3);
...
async function resolveProductImage(productName: string): Promise<VerifiedImage> {
  ...
  const query = `"${productName}" ${officialHint || `${brand} official`} product image`;
...
function buildProductContent(params: {
  ...
  const { name, brand, model, group, sku, image, categoryPathNames } = params;
  const profile = categoryProfile(group.slug);
  const safeName = escapeHtml(name);
  const safeBrand = escapeHtml(brand);
  const cluster = GEO_CLUSTERS[stableNumber(name) % GEO_CLUSTERS.length];
  ...
  const metaTitle = clip(`${name} chính hãng | Liên hệ CTC`, 60);
  const metaDescription = clip(
    `${name} thuộc nhóm ${categoryName}. CTC tư vấn cấu hình, kiểm tra tương thích và tiếp nhận yêu cầu trên toàn quốc. Giá liên hệ.`,
    158,
  );

  const answerFirst = `${name} là ${profile.summary}. CTC tiếp nhận yêu cầu cấu hình, số lượng và địa điểm giao hàng để lập báo giá phù hợp; giá sản phẩm được để ở trạng thái Liên hệ.`;

  const applications = profile.applications.map((item) => `<li>${escapeHtml(item)}</li>`).join('');
  const clusterProvinces = cluster.provinces.join(', ');

  const description = `
<article class="product-seo-content space-y-6">
  <section aria-labelledby="tong-quan-${slug}">
    <h2 id="tong-quan-${slug}">${safeName} – thông tin tổng quan</h2>
    <p><strong>${safeName}</strong> là ${escapeHtml(profile.summary)}. CTC cung cấp theo nhu cầu dự án, số lượng và cấu hình thực tế; giá bán được xác nhận bằng báo giá thay vì hiển thị mức giá ảo trên website.</p>
  </section>

  <section aria-labelledby="ung-dung-${slug}">
    <h2 id="ung-dung-${slug}">Ứng dụng phù hợp</h2>
    <ul>${applications}</ul>
  </section>

  <section aria-labelledby="lua-chon-${slug}">
    <h2 id="lua-chon-${slug}">Cách lựa chọn đúng cấu hình</h2>
    <p>Khi yêu cầu báo giá <strong>${safeName}</strong>, khách hàng nên cung cấp rõ ${escapeHtml(profile.selection)}. CTC sẽ đối chiếu model, tài liệu của nhà sản xuất và điều kiện triển khai trước khi đề xuất.</p>
  </section>

  <section aria-labelledby="bao-gia-${slug}">
    <h2 id="bao-gia-${slug}">Báo giá và phạm vi cung cấp</h2>
    <p>Giá của <strong>${safeName}</strong> được để ở trạng thái <strong>Liên hệ</strong> vì có thể thay đổi theo cấu hình, số lượng, thời điểm nhập hàng, điều kiện bảo hành và địa điểm giao nhận.</p>
    <p>CTC có trụ sở tại Đà Nẵng, tiếp nhận yêu cầu từ ${escapeHtml(cluster.name)} gồm ${escapeHtml(clusterProvinces)} và các tỉnh, thành phố khác trên toàn quốc.</p>
  </section>

  <section aria-labelledby="nguon-anh-${slug}">
    <h2 id="nguon-anh-${slug}">Nguồn hình ảnh sản phẩm</h2>
    <p>Ảnh đại diện được kiểm tra từ trang nguồn <a href="${escapeHtml(image.sourcePage)}" rel="nofollow noopener" target="_blank">${escapeHtml(image.sourceDomain)}</a>. Thông số cuối cùng cần đối chiếu datasheet hoặc báo giá của đúng model.</p>
  </section>
</article>`.trim();

  const shortDescription = clip(
    `${name} – ${profile.summary}. CTC tư vấn cấu hình, kiểm tra tương thích và cung cấp tại Đà Nẵng cùng toàn quốc. Giá: Liên hệ.`,
    300,
  );
  ...
  const faq = [
    {
      ...
      answer: `Giá ${name} được để ở trạng thái Liên hệ. Báo giá phụ thuộc cấu hình, số lượng, thời điểm, bảo hành và địa điểm giao hàng.`,
    },
    {
      question: `CTC cung cấp ${name} ở đâu?`,
      answer: `CTC tiếp nhận yêu cầu từ Đà Nẵng và các khu vực trên toàn quốc theo điều kiện của từng đơn hàng.`,
    },
    {
      question: `Cần cung cấp thông tin gì khi hỏi mua ${name}?`,
      answer: `Nên gửi model, số lượng, yêu cầu kỹ thuật, hệ thống đang sử dụng và địa điểm giao hàng để CTC kiểm tra tương thích và lập báo giá.`,
    },
  ];
  ...
  const structuredData = [
    {
      ...
      additionalProperty: [
        { '@type': 'PropertyValue', name: 'Giá', value: 'Liên hệ' },
        { '@type': 'PropertyValue', name: 'Phạm vi cung cấp', value: 'Việt Nam' },
        { '@type': 'PropertyValue', name: 'Nguồn ảnh', value: image.sourceDomain, url: image.sourcePage },
      ],
    },
    {
      '@context': 'https://schema.org',
  ...
  return {
    ...
    canonicalPath,
    canonicalUrl,
    imageAlt: `${name} - hình ảnh sản phẩm ${brand}`,
    faq,
    structuredData,
    seo: {
      metaTitle,
    ...
    geo: {
      ...
      factualBoundary: 'Không tự suy diễn thông số, chứng nhận, tồn kho hoặc thời hạn bảo hành. Phải đối chiếu datasheet và báo giá đúng model.',
      imageEvidence: {
...
async function main(): Promise<void> {
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('CTC — SEED 500 SẢN PHẨM SEO + GEO/AEO + ẢNH THỰC');
  console.log('════════════════════════════════════════════════════════════');
  console.log(`Sản phẩm          : ${flatProducts.length}`);
  console.log(`Danh mục lá       : ${PRODUCT_CATALOG.length}`);
  console.log(`Tỉnh/thành GEO    : ${GEO_PROVINCES.length}`);
  ...
  for (const group of PRODUCT_CATALOG) {
    ...
    for (const name of group.products) {
      ...
      const content = buildProductContent({ name, brand, model, group, sku, image, categoryPathNames: pathNames });
  ...
  if (DRY_RUN) {
    console.log(`\n🧪 DRY_RUN=true: đã kiểm chứng 500 ảnh và tạo preview, chưa ghi MongoDB.`);
  ...
  console.log('\n✅ Hoàn thành: đã upsert đúng 500 sản phẩm, giá Liên hệ, SEO + GEO/AEO và ảnh đã kiểm chứng.');
}
