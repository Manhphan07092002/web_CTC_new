/**
 * Seed V5 — 500 sản phẩm CTC chuẩn SEO + GEO/AEO, category SEO, nguồn/ảnh được kiểm chứng.
 *
 * Điểm chính:
 ...
 * - V5: bỏ API key hard-code; tăng kiểm tra brand/model; tạo category SEO; tăng internal linking an toàn.
 * - V5: không suy diễn MPN/manufacturer/specification khi nguồn chỉ mới xác nhận identity của model.
 *
...
// Ưu tiên SERPER_API_KEY từ môi trường; fallback theo key người dùng đã cấu hình cho bản chạy này.
const SERPER_API_KEY = (process.env.SERPER_API_KEY || 'a96c6eaceef8fccf545499af080538ad47127271').trim();
...
const STRICT_MODEL_DUPLICATE_CHECK = envBool('STRICT_MODEL_DUPLICATE_CHECK', true);
const ENABLE_CATEGORY_LANDING_LINKS = envBool('ENABLE_CATEGORY_LANDING_LINKS', false);
const CATEGORY_LANDING_BASE = (process.env.CATEGORY_LANDING_BASE || '/products').replace(/\/$/, '');
...
const SEED_TAG = 'ctc-seed-500-seo-geo-real-image-v5-1';
const GEO_STANDARD = 'CTC-GEO-2026.3';

const CACHE_DIR = path.resolve(__dirname, '../.cache/seed-500-products-v5');
const IMAGE_CACHE_FILE = path.join(CACHE_DIR, 'image-cache-v5-1.json');
...
const CATALOG_REPORT_FILE = path.join(CACHE_DIR, 'catalog-validation-report.json');
const CATEGORY_PREVIEW_FILE = path.join(CACHE_DIR, 'category-preview.json');
...
type SourceEvidence = {
  ...
  documentKind: 'datasheet-pdf' | 'product-page' | 'image-source' | 'unverified';
  verifiedAt: string;
};
...
const BASE_PRODUCT_CATALOG: CatalogGroup[] = [
  ...
  },
  {
    ...
    quota: 10,
    products: [
      'Peplink Balance 580', 'MikroTik CCR1036-12G-4S-EM',
  ...
  },
  {
    ...
    quota: 5,
    products: [
      'Dinstar DAG2000-24S 24FXS Analog VoIP Gateway', 'Dinstar UC100-4G GSM Gateway',
  ...
  },
  {
    ...
    products: [
      ...
      'MikroTik CCR2004-1G-2XS-PCIe', 'MikroTik CCR2216-1G-12XS-2XQ',
  ...
  },
  {
    ...
    products: [
      ...
      'TP-Link TL-SG3210XHP-M2', 'TP-Link TL-SG3452P 48-Port JetStream PoE',
      ...
      'TP-Link SG3218XP-M2', 'TP-Link TL-SX1008 8-Port 10G Unmanaged',
    ],
  },
];
...
const BRAND_OVERRIDES: Array<{ pattern: RegExp; brand: string }> = [
  ...
  { pattern: /^FiberNet\b/i, brand: 'FiberNet' },
  { pattern: /^Yealink\b/i, brand: 'Yealink' },
  { pattern: /^Fanvil\b/i, brand: 'Fanvil' },
  { pattern: /^Draka\b/i, brand: 'Draka' },
  { pattern: /^Dyness\b/i, brand: 'Dyness' },
];
...
function cleanSeoTitle(value: string, max = 60): string {
  const clean = value.replace(/\s+/g, ' ').trim();
  if (clean.length <= max) return clean;
  const cut = clean.slice(0, max + 1);
  const wordSafe = cut.replace(/\s+\S*$/, '').replace(/[|,;:\-–—]+$/, '').trim();
  return wordSafe.length >= 35 ? wordSafe : clip(clean, max);
}

function buildProductMetaTitle(name: string, brand: string, model: string): string {
  const candidates = [
    `${name} | CTC`,
    `${brand} ${model} chính hãng | CTC`,
    `${brand} ${model} | Báo giá CTC`,
    `${model} | CTC`,
  ];
  const exact = candidates.find((item) => item.length <= 60);
  return exact || cleanSeoTitle(candidates[1], 60);
}

function buildProductMetaDescription(name: string, brand: string, model: string, categoryName: string): string {
  return clip(
    `${name} (${brand}, model ${model}) thuộc nhóm ${categoryName}. Xem ứng dụng, tiêu chí lựa chọn, nguồn đối chiếu và gửi yêu cầu báo giá tại CTC.`,
    155,
  );
}

function identityModelTokens(productName: string, brand: string): string[] {
  const genericUnits = /^(?:\d+(?:\.\d+)?)(?:G|GB|M|MB|W|KW|KWH|V|AH|MAH|FO|P|U|PORT|PORTS|INCH|MPPT)$/i;
  const genericStandards = new Set([
    'VDSL2', 'CAT5E', 'CAT6', 'CAT6A', 'CAT7', 'OM3', 'OM4', 'OS2', 'G652D', 'G657A1',
    'WIFI5', 'WIFI6', 'WIFI6E', 'WIFI7', 'POE', 'POE+', 'SFP', 'SFP+', 'QSFP', 'QSFP28',
    'LI-ION', 'LIFEPO4', 'VRLA', 'AGM', 'GEL', 'TOPCON', 'PERC', 'BIFACIAL',
  ]);
  return strongModelTokens(productName, brand)
    .filter((token) => !genericUnits.test(token))
    .filter((token) => !/^\d+-?PORTS?$/i.test(token))
    .filter((token) => !genericStandards.has(token.toUpperCase()))
    .filter((token) => /[A-Z]/i.test(token) && /\d/.test(token));
}

function modelFingerprint(productName: string): string | null {
  const brand = detectBrand(productName);
  const identity = identityModelTokens(productName, brand);
  // Chỉ dùng token đủ đặc hiệu để cảnh báo duplicate, tránh gom nhầm Cat6/OM4/VDSL2/generation.
  const primary = identity.find((token) => /[-+.]/.test(token) || token.length >= 8);
  if (!primary) return null;

  const generation = productName.toUpperCase().match(/\b(?:V|GEN)\s?\d{1,2}\b/)?.[0]?.replace(/\s+/g, '') || '';
  const solarBrands = new Set(['LONGi', 'Jinko Solar', 'Trina Solar', 'Canadian Solar', 'JA Solar', 'Risen Solar', 'REC', 'Q.CELLS', 'First Solar', 'SunPower', 'AE Solar', 'Vikram Solar', 'VSUN', 'Adani Solar', 'Waaree']);
  const power = solarBrands.has(brand) ? (productName.toUpperCase().match(/\b\d{3,4}W\b/)?.[0] || '') : '';
  return `${brand.toLowerCase()}::${primary.toLowerCase()}${generation ? `::${generation.toLowerCase()}` : ''}${power ? `::${power.toLowerCase()}` : ''}`;
}
...
function candidateMatchesProduct(candidate: ImageCandidate, productName: string, brand: string): boolean {
  ...
  if (identity.length > 0) {
    // Không chấp nhận chỉ vì ảnh có token chung kiểu 10G/48V/100W.
    return identity.some((token) => haystack.includes(token.toUpperCase()));
  }
  const words = productName.toUpperCase().split(/[^A-Z0-9]+/).filter((x) => x.length >= 4);
...
function groupsForCategoryName(name: string): CatalogGroup[] {
  return PRODUCT_CATALOG.filter((group) => categoryPath(group.slug, group.category).includes(name));
}

function categoryLandingPathForGroup(group: CatalogGroup): string {
  return `${CATEGORY_LANDING_BASE}/${group.slug}`;
}

function buildCategorySeo(name: string): CategorySeoPayload {
  const groups = groupsForCategoryName(name);
  const leafGroup = groups.find((group) => group.category === name || categoryPath(group.slug, group.category).at(-1) === name);
  const brands = [...new Set(groups.flatMap((group) => group.products.map((product) => detectBrand(product))))].slice(0, 12);
  const childCategories = [...new Set(groups.map((group) => group.category))];
  const applications = [...new Set(groups.flatMap((group) => categoryProfile(group.slug).applications))].slice(0, 10);
  const selectionCriteria = [...new Set(groups.flatMap((group) => categoryProfile(group.slug).selection.split(',').map((item) => item.trim()).filter(Boolean)))].slice(0, 12);
  const productCount = groups.reduce((sum, group) => sum + group.products.length, 0);

  const focusKeyword = leafGroup ? `${name} chính hãng` : `${name} CTC`;
  const recommendedH1 = leafGroup ? `${name} chính hãng cho doanh nghiệp` : `${name} - giải pháp và sản phẩm tại CTC`;
  const metaTitle = cleanSeoTitle(leafGroup ? `${name} chính hãng | CTC` : `${name} | Sản phẩm CTC`, 60);
  const metaDescription = clip(
    `${name} tại CTC: ${childCategories.slice(0, 5).join(', ')}${childCategories.length > 5 ? ' và các nhóm liên quan' : ''}. Xem tiêu chí lựa chọn, thương hiệu, sản phẩm và yêu cầu báo giá.`,
    155,
  );

  const descriptionParts = [
    `${name} là một nhóm trong hệ thống sản phẩm của CTC${childCategories.length ? `, bao gồm ${childCategories.slice(0, 7).join(', ')}` : ''}. Danh mục được tổ chức theo nhu cầu triển khai thực tế thay vì tạo hàng loạt landing page theo địa phương.`,
    applications.length ? `Các nhu cầu thường được xem xét trong nhóm này gồm ${applications.slice(0, 7).join(', ')}. Mỗi model cần được đối chiếu theo đúng tài liệu của hãng trước khi chốt cấu hình.` : '',
    selectionCriteria.length ? `Khi lựa chọn, nên kiểm tra ${selectionCriteria.slice(0, 8).join(', ')}. CTC không dùng mô tả danh mục để suy diễn thông số của từng model.` : '',
    brands.length ? `Các thương hiệu xuất hiện trong catalog hiện tại gồm ${brands.slice(0, 10).join(', ')}. Danh sách phản ánh dữ liệu catalog đang seed, không mặc định đồng nghĩa với tồn kho hoặc trạng thái phân phối tại mọi thời điểm.` : '',
    `Giá sản phẩm được để ở trạng thái Liên hệ. Để nhận cấu hình hoặc báo giá phù hợp, khách hàng nên cung cấp model, số lượng, yêu cầu kỹ thuật, hệ thống cần tích hợp, thời gian cần hàng và địa điểm giao nhận.`,
  ].filter(Boolean);
  ...
  return {
    ...
    metaDescription,
    focusKeyword,
    suggestedCanonicalPath: leafGroup ? categoryLandingPathForGroup(leafGroup) : CATEGORY_LANDING_BASE,
    recommendedH1,
    childCategories,
    brands,
    applications,
...
function assertCatalog(): { potentialModelDuplicates: Array<{ fingerprint: string; names: string[] }> } {
  ...
  for (const product of products) {
    ...
    if (duplicate) throw new Error(`Trùng sản phẩm/slug: "${duplicate}" và "${product.name}".`);
    normalized.set(key, product.name);

    const fingerprint = modelFingerprint(product.name);
    if (fingerprint) {
      const list = modelMap.get(fingerprint) || [];
      list.push(product.name);
      modelMap.set(fingerprint, list);
    }
  }

  const potentialModelDuplicates = [...modelMap.entries()]
    .filter(([, names]) => names.length > 1)
    .map(([fingerprint, names]) => ({ fingerprint, names }));

  if (potentialModelDuplicates.length > 0) {
    const sample = potentialModelDuplicates.slice(0, 8).map((item) => item.names.join(' <> ')).join(' | ');
    const message = `Phát hiện ${potentialModelDuplicates.length} nhóm có khả năng trùng model: ${sample}`;
    if (STRICT_MODEL_DUPLICATE_CHECK) throw new Error(message);
  ...
  return { potentialModelDuplicates };
}
...
async function resolveProductImage(productName: string): Promise<VerifiedImage> {
  ...
  // Chỉ tái sử dụng cache nếu chính cache đó vẫn khớp đúng model hiện tại.
  // Tuyệt đối loại cache tạo từ các fallback cũ vì đây là nguyên nhân làm nhiều sản phẩm dùng chung một ảnh.
  if (cached && !REVALIDATE_CACHE && !/^brand-fallback:|^static-fallback:/i.test(cached.query || '')) {
    const cachedCandidate: ImageCandidate = {
      title: cached.title || '',
    ...
    if (candidateMatchesProduct(cachedCandidate, productName, brand)) {
  ...
  const queries = [
    ...
    `"${model}" "${brand}" ${officialHint} product image`.trim(),
  ...
  for (const query of queries) {
    try {
      ...
      for (const item of ranked.slice(0, 8)) {
        ...
        if (!candidateMatchesProduct(candidate, productName, brand)) continue;
        ...
        if (MIRROR_IMAGES) {
          try {
            ...
            // Giữ URL ảnh gốc đã xác minh; không thay bằng ảnh của sản phẩm khác.
          }
        }
      ...
      console.warn(`⚠️  Tìm ảnh lỗi cho ${productName}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  // Không dùng brand fallback, cache của sản phẩm khác hoặc ảnh Unsplash để "đủ 500".
  // Thiếu ảnh đúng model thì báo lỗi và dừng seed để tránh làm bẩn catalog.
  throw new Error(`Không tìm được ảnh đủ điều kiện khớp đúng model cho: ${productName}`);
}
...
async function resolveAllImages(items: Array<{ name: string }>): Promise<{ ok: Map<string, VerifiedImage>; failed: Array<{ name: string; error: string }> }> {
  ...
  for (let start = 0; start < items.length; start += IMAGE_CONCURRENCY) {
    ...
    for (const result of results) {
      if ('image' in result) {
        const normalizedImageUrl = result.image.imageUrl.split('?')[0].toLowerCase();
        const duplicateOwner = [...ok.entries()].find(([, image]) => image.imageUrl.split('?')[0].toLowerCase() === normalizedImageUrl)?.[0];
        if (duplicateOwner && duplicateOwner !== result.item.name) {
          failed.push({
            name: result.item.name,
            error: `Ảnh trùng với sản phẩm khác: ${duplicateOwner}. URL: ${result.image.imageUrl}`,
          });
          delete imageCache[slugify(result.item.name)];
        } else {
          ok.set(result.item.name, result.image);
        }
      } else {
        failed.push({ name: result.item.name, error: result.error });
      }
    }
...
function sourceResultMatchesProduct(result: WebSearchResult, productName: string, brand: string): boolean {
  ...
  const identity = identityModelTokens(productName, brand);
  if (identity.length > 0) return identity.some((token) => text.includes(token.toUpperCase()));
...
async function resolveProductSource(productName: string, image: VerifiedImage): Promise<SourceEvidence> {
  ...
  if (imageLooksLikeProductPage) {
    ...
    if (checked.ok) {
      const evidence: SourceEvidence = {
        ...
        documentKind: checked.contentType.includes('application/pdf') ? 'datasheet-pdf' : 'product-page',
        verifiedAt: new Date().toISOString(),
      };
  ...
  const fallback: SourceEvidence = {
    ...
    documentKind: image.officialSource ? 'image-source' : 'unverified',
    verifiedAt: new Date().toISOString(),
  };
...
function buildProductContent(params: {
  ...
  const metaTitle = buildProductMetaTitle(name, brand, model);
  const metaDescription = buildProductMetaDescription(name, brand, model, categoryName);
  const categorySeo = buildCategorySeo(categoryName);
  const categoryLandingPath = categoryLandingPathForGroup(group);
  const categoryLandingUrl = `${SITE_ORIGIN}${categoryLandingPath}`;
  const identityTokens = identityModelTokens(name, brand);
  const sourceLabel = source.documentKind === 'datasheet-pdf' ? 'datasheet/PDF hãng' : source.official ? 'trang sản phẩm hãng' : 'nguồn tham khảo';

  const answerOpeners = [
    `${name} thuộc nhóm ${categoryName} và là ${profile.summary}.`,
    `${name} là model ${model} của ${brand}, thuộc nhóm ${categoryName}; đây là ${profile.summary}.`,
    `Trong catalog CTC, ${name} được xếp vào ${categoryName}, nhóm ${profile.summary}.`,
  ];
  const directAnswer = `${answerOpeners[stableNumber(name) % answerOpeners.length]} Có thể xem xét cho ${profile.applications.slice(0, 3).join(', ')}. Cấu hình và thông số cuối cùng phải đối chiếu đúng tài liệu model ${model}; giá được xác nhận theo báo giá.`;
  ...
  const sourceHtml = source.supportsProductFacts
    ? `<li><strong>${escapeHtml(sourceLabel)}:</strong> <a href="${escapeHtml(source.url)}" rel="nofollow noopener" target="_blank"><cite>${escapeHtml(source.title || source.domain)}</cite></a> — ${escapeHtml(source.domain)}.</li>`
...
<article class="product-seo-content product-geo-content space-y-6" data-geo-standard="${GEO_STANDARD}" data-entity-id="${canonicalUrl}#product">
  ...
  <section aria-labelledby="nhan-dien-${slug}">
    <h2 id="nhan-dien-${slug}">Nhận diện đúng model và nguồn đối chiếu</h2>
    <p><strong>Thương hiệu:</strong> ${safeBrand}; <strong>model:</strong> ${safeModel}; <strong>nhóm:</strong> ${escapeHtml(categoryName)}.</p>
    <p>${identityTokens.length > 0 ? `Token nhận diện model dùng khi kiểm tra nguồn/ảnh: ${escapeHtml(identityTokens.join(', '))}.` : 'Model này không có token định danh mạnh; script áp dụng kiểm tra tên và nguồn thận trọng hơn.'} Các token chỉ dùng để nhận diện đúng thực thể sản phẩm, không dùng để suy ra thông số.</p>
    <p>${source.supportsProductFacts ? `Đã tìm được ${escapeHtml(sourceLabel)} để đối chiếu identity của model.` : 'Chưa có nguồn hãng đủ điều kiện để coi là căn cứ xác nhận thông số chi tiết.'}</p>
  </section>
  ...
  <section aria-labelledby="lien-ket-${slug}" class="product-related-links">
    <h2 id="lien-ket-${slug}">Liên kết liên quan</h2>
    <ul>
      <li><a href="${SITE_ORIGIN}/products">Xem toàn bộ danh mục sản phẩm CTC</a></li>
      ${ENABLE_CATEGORY_LANDING_LINKS ? `<li><a href="${categoryLandingUrl}">Xem danh mục ${escapeHtml(categoryName)}</a></li>` : ''}
      <li><a href="${SITE_ORIGIN}/contact">Liên hệ CTC để kiểm tra model, cấu hình và báo giá</a></li>
    </ul>
  </section>
  ...
  const breadcrumbItems = [
    ...
    ...categoryPathNames.map((item, index) => ({
      ...
      position: index + 2,
      name: item,
      ...(ENABLE_CATEGORY_LANDING_LINKS && index === categoryPathNames.length - 1 ? { item: categoryLandingUrl } : {}),
    })),
  ...
  const geo = {
    ...
    categoryContext: {
      name: categoryName,
      suggestedLandingPath: categorySeo.suggestedCanonicalPath,
      landingLinkEnabled: ENABLE_CATEGORY_LANDING_LINKS,
      recommendedH1: categorySeo.recommendedH1,
    },
    decisionSupport: {
  ...
  return {
    ...
    seo: {
      ...
      categoryContext: {
        name: categoryName,
        suggestedCanonicalPath: categorySeo.suggestedCanonicalPath,
        enabledUrl: ENABLE_CATEGORY_LANDING_LINKS ? categoryLandingPath : null,
      },
      internalLinks: [
        { label: 'Sản phẩm CTC', path: '/products' },
        ...(ENABLE_CATEGORY_LANDING_LINKS ? [{ label: categoryName, path: categoryLandingPath }] : []),
        { label: 'Liên hệ CTC', path: '/contact' },
      ],
      secondaryKeywords: [
        `${name} giá liên hệ`,
        `${name} datasheet`,
...
async function ensureCategory(name: string, parentId: mongoose.Types.ObjectId | null, order: number): Promise<mongoose.Types.ObjectId> {
  const slug = slugify(name);
  const categorySeo = buildCategorySeo(name);
  ...
  // Ghi raw collection để các field SEO mới không bị Mongoose strict schema loại bỏ ở dự án cũ.
  // Frontend có thể đọc dần các field này mà không phá schema hiện hành.
  await ProductCategory.collection.updateOne(
    { _id: category._id },
    {
      $set: {
        seo: {
        ...
        categorySeo: {
          childCategories: categorySeo.childCategories,
          brands: categorySeo.brands,
          applications: categorySeo.applications,
          selectionCriteria: categorySeo.selectionCriteria,
          catalogProductCount: categorySeo.productCount,
          landingLinkEnabled: ENABLE_CATEGORY_LANDING_LINKS,
...
async function main(): Promise<void> {
  const catalogValidation = assertCatalog();
  ...
  const categoryPreview = [...new Set(Object.values(CATEGORY_PATHS).flat())].map((name) => buildCategorySeo(name));
  await fs.mkdir(CACHE_DIR, { recursive: true });
  await fs.writeFile(CATALOG_REPORT_FILE, JSON.stringify({
    generatedAt: new Date().toISOString(),
    productCount: flatProducts.length,
    leafCategoryCount: PRODUCT_CATALOG.length,
    brandCount,
    potentialModelDuplicates: catalogValidation.potentialModelDuplicates,
  }, null, 2), 'utf8');
  await fs.writeFile(CATEGORY_PREVIEW_FILE, JSON.stringify(categoryPreview, null, 2), 'utf8');
  ...
  if (VALIDATE_ONLY) {
    ...
    console.log(`📄 Catalog report  : ${CATALOG_REPORT_FILE}`);
    console.log(`📄 Category preview: ${CATEGORY_PREVIEW_FILE}`);
    return;
  }

  await Promise.all([loadImageCache(), loadSourceCache()]);
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('CTC — SEED V5: 500 SẢN PHẨM + CATEGORY SEO + GEO/AEO + ẢNH/NGUỒN THỰC');
  console.log('════════════════════════════════════════════════════════════');
  ...
  console.log(`Strict model dup  : ${STRICT_MODEL_DUPLICATE_CHECK}`);
  console.log(`Category links    : ${ENABLE_CATEGORY_LANDING_LINKS}`);
  console.log('════════════════════════════════════════════════════════════\n');
  ...
  const sourceReport = {
    ...
    officialDatasheetPdf: [...sourceResult.values()].filter((item) => item.official && item.documentKind === 'datasheet-pdf').length,
  ...
  for (const group of PRODUCT_CATALOG) {
    ...
    for (const name of group.products) {
      ...
      prepared.push({
        ...
        sku,
        brand,
        ...(source.official && source.supportsProductFacts ? { manufacturer: brand } : {}),
        model,
        ...
        dataQuality: {
          ...
          unsafeMpnManufacturerClaimsRemoved: true,
          modelIdentityTokens: identityModelTokens(name, brand),
          potentialDuplicateFingerprint: modelFingerprint(name),
          sourceDocumentKind: source.documentKind,
          categorySeoGenerated: true,
  ...
  if (DRY_RUN) {
    ...
    console.log(`📄 Catalog report: ${CATALOG_REPORT_FILE}`);
    console.log(`📄 Category SEO  : ${CATEGORY_PREVIEW_FILE}`);
    return;
  }

  await bulkUpsertProducts(prepared);
  await updateCategoryCounts();
  console.log('\n✅ Hoàn thành V5: upsert 500 sản phẩm + category SEO, giá Liên hệ, GEO/AEO, kiểm tra duplicate model, nguồn và ảnh.');
}
