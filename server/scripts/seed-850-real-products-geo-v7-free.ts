/**
 * Seed/Crawler V7 Free Mode: mục tiêu 850 sản phẩm thật đang có trên thị trường,
 ...
 * - Mặc định không cần API trả phí: sitemap -> category/pagination -> API catalog công khai -> HTML/JSON-LD.
 * - Serper chỉ là fallback tùy chọn, tắt mặc định; không nhúng API key vào mã nguồn.
 * - Có thể bật Playwright miễn phí để đọc website render bằng JavaScript.
 ...
 *
 * Quét miễn phí và chỉ ghi manifest/report (khuyến nghị chạy trước):
 *   DISCOVERY_ONLY=true FREE_DISCOVERY_MODE=true ENABLE_SERPER=false \
 *     npx tsx server/scripts/seed-850-real-products-geo-v7-free.ts
 *
 * Bật Playwright nếu website hãng cần JavaScript (cài package trước):
 *   PLAYWRIGHT_FALLBACK=true npx tsx server/scripts/seed-850-real-products-geo-v7-free.ts
 *
 * Serper fallback tùy chọn:
 *   ENABLE_SERPER=true SERPER_API_KEY=... npx tsx server/scripts/seed-850-real-products-geo-v7-free.ts
 */
...
// Không bao giờ nhúng khóa vào mã nguồn. V7 chạy được mà không cần Serper.
...
const FREE_DISCOVERY_MODE = envBool('FREE_DISCOVERY_MODE', true);
const ENABLE_SERPER = envBool('ENABLE_SERPER', false);
...
const DISCOVERY_PARSER_VERSION = '7.0.0-free';
const SEED_TAG = 'ctc-seed-850-real-products-geo-evidence-v7-free';
const GEO_STANDARD = 'CTC-GEO-2026.3.1';

const CACHE_DIR = path.resolve(__dirname, '../.cache/seed-850-products-v7-free');
const DISCOVERY_CACHE_FILE = path.join(CACHE_DIR, 'discovery-cache-v7.json');
const DISCOVERY_SCAN_CACHE_FILE = path.join(CACHE_DIR, 'discovery-scan-cache-v7.json');
const DISCOVERY_REPORT_FILE = path.join(CACHE_DIR, 'discovery-report-v7.json');
const DISCOVERED_CATALOG_FILE = path.join(CACHE_DIR, 'discovered-catalog-v7.json');
const IMAGE_CACHE_FILE = path.join(CACHE_DIR, 'image-cache-v7.json');
const EVIDENCE_CACHE_FILE = path.join(CACHE_DIR, 'evidence-cache-v7.json');
...
// V5 được giữ lại trong file như catalog tham chiếu. V7 chỉ seed catalog đã
...
function assertCatalog(): void {
  ...
  if (products.length < MIN_TOTAL_PRODUCTS || products.length > MAX_TOTAL_PRODUCTS) {
    throw new Error(
      `Catalog V7 phải nằm trong ${MIN_TOTAL_PRODUCTS}–${MAX_TOTAL_PRODUCTS} sản phẩm thật, hiện có ${products.length}. ` +
...
function assertTargetConfig(): void {
  ...
  if (CATEGORY_TARGETS.length !== 54) {
    throw new Error(`Cấu hình V7 phải có 54 danh mục lá, hiện có ${CATEGORY_TARGETS.length}.`);
  }
  ...
  for (const item of CATEGORY_TARGETS) {
    if (slugSet.has(item.slug)) throw new Error(`Trùng category slug V7: ${item.slug}`);
...
async function main(): Promise<void> {
  ...
  if (VALIDATE_CONFIG_ONLY) {
    ...
    console.log(`✅ Cấu hình V7 Free Mode hợp lệ: ${CATEGORY_TARGETS.length} danh mục lá, quota ${TARGET_TOTAL_PRODUCTS}.`);
    console.log(JSON.stringify(totals, null, 2));
    console.log(`🆓 Sitemap: ${DISCOVERY_USE_SITEMAPS}; category pages: ${DISCOVERY_USE_CATEGORY_PAGES}; public catalog API: ${DISCOVERY_USE_PUBLIC_CATALOG_API}.`);
    console.log(`🔎 Serper fallback: ${SERPER_ENABLED ? `bật, trần ${MAX_DISCOVERY_QUERIES}; ước tính ${estimatedDiscoveryRequests()}` : 'tắt'}.`);
    console.log(`🎭 Playwright fallback: ${PLAYWRIGHT_FALLBACK ? 'bật' : 'tắt'}.`);
    return;
  }
  ...
  if (DISCOVERY_ONLY) {
    ...
    console.log(`\n🧭 DISCOVERY_ONLY hoàn tất: ${discoveredTotal}/${TARGET_TOTAL_PRODUCTS} sản phẩm thật từ nguồn hãng.`);
    console.log(`📄 Manifest: ${DISCOVERED_CATALOG_FILE}`);
    console.log(`📄 Báo cáo : ${DISCOVERY_REPORT_FILE}`);
    if (discoveredTotal < MIN_TOTAL_PRODUCTS) {
      console.warn(`⚠️  Chưa đạt ngưỡng seed ${MIN_TOTAL_PRODUCTS}; tiếp tục chạy lại để tận dụng cache hoặc bật Playwright/Serper fallback.`);
    }
    return;
  }
  assertCatalog();
  ...
  console.log('\n════════════════════════════════════════════════════════════');
  console.log('CTC — V7 FREE MODE: 850 SẢN PHẨM THẬT + CATEGORY SEO + EVIDENCE + ẢNH ĐÚNG MODEL');
  console.log('════════════════════════════════════════════════════════════');
  ...
  console.log(`Query Serper      : ${discoveryQueryCount}/${MAX_DISCOVERY_QUERIES}`);
  console.log(`Nguồn miễn phí    : sitemap=${DISCOVERY_USE_SITEMAPS}, category=${DISCOVERY_USE_CATEGORY_PAGES}, public API=${DISCOVERY_USE_PUBLIC_CATALOG_API}`);
  ...
  console.log(`Serper fallback   : ${SERPER_ENABLED ? 'BẬT' : 'TẮT'}`);
  console.log(`Playwright        : ${PLAYWRIGHT_FALLBACK ? 'BẬT' : 'TẮT'}`);
  ...
  console.log(`\n✅ Hoàn thành V7: upsert ${prepared.length} sản phẩm thật, Category SEO, evidence/datasheet/specs và ảnh đúng model.`);
}
