...
const NewsForm: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto pb-20">
      {/* === HEADER === */}
      <div className="flex items-center justify-between mb-6 sticky top-0 z-30 bg-gray-50 dark:bg-slate-900/90 py-4 border-b border-gray-200 dark:border-slate-700/80 backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <button onClick={handleExit} className="p-2 hover:bg-gray-100 dark:hover:bg-slate-800 rounded-lg transition-colors text-gray-500 dark:text-gray-400 cursor-pointer">
            <X size={20} />
          </button>
          <div>
        ...
        <div className="flex items-center gap-2">
          {/* Status selector */}
          <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-xs">
            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 hidden sm:inline">Trạng thái:</span>
            <select
              value={formData.status}
              onChange={e => setFormData({ ...formData, status: e.target.value as any })}
              className={`text-xs font-black outline-none bg-transparent cursor-pointer ${
                formData.status === 'published' ? 'text-emerald-600 dark:text-emerald-400' :
                formData.status === 'pending' ? 'text-amber-600 dark:text-amber-400 font-extrabold' : 'text-slate-600 dark:text-slate-300'
              }`}
            >
              <option value="published" className="dark:bg-slate-800 dark:text-gray-100">🟢 Đã xuất bản (Published)</option>
              <option value="pending" className="dark:bg-slate-800 dark:text-gray-100">🟡 Chờ duyệt (Pending)</option>
              <option value="draft" className="dark:bg-slate-800 dark:text-gray-100">📝 Bản nháp (Draft)</option>
            </select>
          </div>
          ...
          {formData.status !== 'published' ? (
            ...
            isEdit && id && (
              <button
                ...
                disabled={indexing}
                className="flex items-center gap-1.5 px-3.5 py-2 bg-emerald-50 dark:bg-emerald-950/40 hover:bg-emerald-100 dark:hover:bg-emerald-900/60 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 rounded-xl text-sm font-bold transition-all disabled:opacity-50 shadow-sm"
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-semibold border transition-all ${
              preview ? 'bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400 border-blue-200 dark:border-blue-800/60' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-gray-300 border-gray-200 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700'
            }`}
          >
          ...
          <button
            ...
            disabled={saving}
            className="flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-900 dark:bg-slate-700 dark:hover:bg-slate-600 text-white rounded-xl font-bold text-sm transition-all shadow-sm disabled:opacity-50"
          >
            {saving ? (
      ...
      {preview ? (
        /* === PREVIEW MODE === */
        <div className="bg-white dark:bg-slate-800/90 rounded-2xl shadow border border-gray-100 dark:border-slate-700/60 p-8 md:p-12">
          <div className="max-w-3xl mx-auto">
            ...
            <h1 className="text-3xl md:text-4xl font-black text-gray-900 dark:text-gray-100 leading-tight mb-4">
            ...
            {formData.excerpt && (
              <p className="text-lg text-gray-600 dark:text-gray-300 italic border-l-4 border-primary pl-4 mb-8 leading-relaxed">{formData.excerpt}</p>
            )}
            <div
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="bg-white dark:bg-slate-800/90 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/60 p-6">
                <label className="block text-xs font-black text-gray-400 dark:text-gray-400 uppercase tracking-wider mb-2">
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full text-2xl font-black text-gray-900 dark:text-gray-100 border-0 outline-none resize-none placeholder-gray-300 dark:placeholder-gray-600 bg-transparent leading-tight"
                  placeholder="Nhập tiêu đề bài viết ấn tượng..."
                />
                <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">{formData.title.length}/200 ký tự</div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800/90 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/60 p-6">
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full border-0 outline-none resize-none text-gray-600 dark:text-gray-300 leading-relaxed placeholder-gray-300 dark:placeholder-gray-600 bg-transparent text-sm"
                  placeholder="Tóm tắt nội dung chính, hiển thị trên trang danh sách tin tức (1-2 câu)..."
                />
                <div className="text-xs text-gray-400 dark:text-gray-500 mt-1">{formData.excerpt.length}/500 ký tự</div>
              </div>

              {/* Rich Text Editor */}
              <div className="bg-white dark:bg-slate-800/90 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/60 p-6">
                <div className="flex items-center justify-between mb-3">
                  <label className="block text-xs font-black text-gray-400 dark:text-gray-400 uppercase tracking-wider">
                    📄 NỘI DUNG CHI TIẾT
                  </label>
                ...
                <Suspense fallback={
                  <div className="border border-gray-200 dark:border-slate-700 rounded-xl h-80 flex items-center justify-center bg-gray-50 dark:bg-slate-900">
                    <div className="text-center">
                      <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary mx-auto mb-2" />
                      <p className="text-sm text-gray-400 dark:text-gray-500">Đang tải editor...</p>
                    </div>
                  </div>
                }>
                  <RichTextEditor
            ...
            <div className="space-y-5">
                ...
                <div className="space-y-3">
                  {formData.image ? (
                    <div className="relative group">
                      <img
                        src={formData.image}
                        alt="Preview"
                        className="w-full h-44 object-cover rounded-xl border border-gray-200 dark:border-slate-700"
                      />
                      <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 rounded-xl transition-opacity flex items-center justify-center">
                        <button
                          ...
                          className="bg-white dark:bg-slate-800 text-gray-800 dark:text-gray-100 text-xs font-bold px-3 py-2 rounded-lg"
                        >
                          Đổi ảnh
                        </button>
                      </div>
                  ...
                  <button
                    ...
                    className={`w-full border-2 border-dashed rounded-xl flex items-center justify-center transition-colors ${
                      formData.image
                        ? 'h-10 border-gray-200 dark:border-slate-700 text-xs text-gray-400 dark:text-gray-500 hover:border-primary hover:text-primary'
                        : 'h-44 border-gray-300 dark:border-slate-700 hover:border-primary hover:bg-primary/5 dark:hover:bg-primary/10'
                    }`}
                  >
                    {formData.image ? (
                      '+ Thay ảnh khác'
                      ...
                      <div className="text-center">
                        ...
                        <span className="text-sm text-gray-400 dark:text-gray-500 font-medium">Chọn ảnh đại diện</span>
                        <p className="text-xs text-gray-300 dark:text-gray-600 mt-1">PNG, JPG, WebP</p>
                      </div>
                    )}
                  </button>
                </div>
              ...
              {/* Meta */}
              <div className="bg-white dark:bg-slate-800/90 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/60 p-5 space-y-4">
                <label className="block text-xs font-black text-gray-400 dark:text-gray-400 uppercase tracking-wider mb-1">
                  ⚙️ THÔNG TIN BÀI VIẾT
                </label>

                {/* Category */}
                <div>
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 flex items-center gap-1 mb-1.5">
                    <Hash size={11} /> Danh mục
                  </label>
                  <select
                    value={formData.categoryId}
                    onChange={(e) => handleCategoryChange(e.target.value)}
                    className="w-full border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-gray-100"
                  >
                ...
                <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-slate-700">
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 flex items-center gap-1 cursor-pointer">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors ${
                      formData.isFeatured ? 'bg-yellow-400' : 'bg-gray-200 dark:bg-slate-700'
                    }`}
                  >
                    <span className={`inline-block h-3.5 w-3.5 transform rounded-full bg-white dark:bg-slate-200 shadow transition-transform ${
              ...
              <div className="bg-white dark:bg-slate-800/90 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700/60 p-5">
                ...
                <div className="flex gap-2 mb-3">
                  <input
                    ...
                    className="flex-1 border border-gray-200 dark:border-slate-700 rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-gray-100"
                      ...
                      return (
                        <span
                          ...
                          className={`inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold rounded-full border transition-all ${
                            isCurrentFocus
                              ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border-emerald-300 dark:border-emerald-700 font-extrabold shadow-sm'
                              : 'bg-primary/10 dark:bg-primary/20 text-primary dark:text-primary-light border-primary/20 dark:border-primary/30 hover:bg-primary/20'
                          }`}
                        >
                          #{tag}
                          <button
                            ...
                            className="text-[10px] bg-white/70 dark:bg-slate-800/80 px-1 py-0.5 rounded text-slate-600 dark:text-slate-300 hover:text-primary font-bold ml-0.5"
